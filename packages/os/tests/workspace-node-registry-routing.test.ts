...
describe('multi-node connector routing', () => {
  ...
  it('should preserve managed-cloud GCP auth when retrying stale lifecycle with installed local trust', async () => {
    const { spawnSync } = await import('node:child_process');
    const { existsSync, mkdirSync, mkdtempSync, readFileSync, rmSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const home = mkdtempSync(join(tmpdir(), 'consuelo-legacy-local-gcp-trust-'));
    try {
      const runtimeDir = join(home, 'runtime');
      const lifecycleDir = join(runtimeDir, 'current', 'scripts');
      mkdirSync(lifecycleDir, { recursive: true });
      ...
      const lifecycleMarker = join(home, 'legacy-lifecycle-local-gcp-trust.json');
      writeFileSync(
        join(lifecycleDir, 'lifecycle.ts'),
        `await Bun.write(${JSON.stringify(lifecycleMarker)}, JSON.stringify({ baseUrl: process.env.CONSUELO_RELEASE_BASE_URL, keys: process.env.CONSUELO_RELEASE_PUBLIC_KEYS_JSON, gcpAuth: process.env.CONSUELO_RELEASE_GCP_METADATA_AUTH ?? null }));`,
        'utf8',
  ...
  it('should reject stale lifecycle GCP auth outside the managed-cloud release origin', async () => {
    const { spawnSync } = await import('node:child_process');
    const { existsSync, mkdirSync, mkdtempSync, rmSync, writeFileSync } = await import('node:fs');
    const { tmpdir } = await import('node:os');
    const { join } = await import('node:path');
    const home = mkdtempSync(join(tmpdir(), 'consuelo-legacy-invalid-gcp-auth-'));
    try {
      const runtimeDir = join(home, 'runtime');
      const lifecycleDir = join(runtimeDir, 'current', 'scripts');
      mkdirSync(lifecycleDir, { recursive: true });
      ...
      const lifecycleMarker = join(home, 'legacy-lifecycle-invalid-gcp-auth');
      writeFileSync(
        join(lifecycleDir, 'lifecycle.ts'),
      ...
      const child = spawnSync(
        ...
        ['-e', `globalThis.fetch=async()=>{throw new Error("metadata should not be read");};${inlineScript}`],
        {
          ...
          env: {
            ...process.env,
            CONSUELO_HOME: home,
            CONSUELO_RELEASE_BASE_URL: 'https://install.consuelohq.com/os/releases',
            CONSUELO_RELEASE_GCP_METADATA_AUTH: '1',
          },
        },
      );
