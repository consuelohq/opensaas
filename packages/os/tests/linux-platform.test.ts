...
describe('Linux platform adapter', () => {
  ...
  it('fails before service mutation for unsupported OS, architecture, or libc', async () => {
    for (const host of [
      ...
      },
      {
        ...
        architecture: 's390x' as const,
        libc: 'glibc' as const,
      },
      {
        platform: 'linux' as const,
        architecture: 'x64' as const,
        libc: 'unknown' as const,
      },
    ]) {
  ...
  it('renders and installs a strict systemd user service using immutable runtime/current', async () => {
    const adapter = createLinuxPlatformAdapter({
      ...
      run: runner([{ exitCode: 0 }, { exitCode: 0 }, { exitCode: 0 }]),
      bunExecutable: '/opt/consuelo/bin/bun',
    });
    ...
    expect(readFileSync(paths.unitPath, 'utf8')).toBe(
      renderSystemdUserUnit({
        home,
        bunExecutable: '/opt/consuelo/bin/bun',
      }),
    ...
    expect(readFileSync(paths.unitPath, 'utf8')).toContain(
      `${home}/runtime/current/scripts/server/main.ts`,
    );
  ...
  it('uses a bounded session-process fallback when systemd user services are unavailable', async () => {
    ...
    const paths = resolveLinuxPlatformPaths(home);
    expect(
      JSON.parse(readFileSync(paths.sessionStatePath, 'utf8')),
  ...
  it('reports structured diagnostics and removes only Consuelo-owned service artifacts', async () => {
    ...
    writeFileSync(
      join(home, 'workspaces', 'user-workspace', 'note.md'),
      'keep\n',
    );
    const adapter = createLinuxPlatformAdapter({
      home,
    ...
    expect(
      readFileSync(join(paths.systemdUserDir, 'user-owned.service'), 'utf8'),
    ).toBe('keep\n');
    expect(
      readFileSync(
        join(home, 'workspaces', 'user-workspace', 'note.md'),
