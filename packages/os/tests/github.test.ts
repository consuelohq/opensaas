...
describe('os github typed facade script', () => {
  ...
  it('builds bounded packets for large pr.view review payloads', () => {
    ...
    const files = Array.from({ length: 240 }, (_, index) => ({
      path: `packages/os/generated-file-${index}.ts`,
      filename: `packages/os/generated-file-${index}.ts`,
      additions: index + 1,
      deletions: index % 3,
      changes: index + 4,
      patch: `+${'x'.repeat(1000)}`,
    ...
    const data = {
      number: 1042,
      title: 'bound os github response packets',
      url: 'https://github.com/consuelohq/opensaas/pull/1042',
      headRefName: 'task/os/bound-os-github-response-packets',
    ...
    const result = createGithubOutput(
      ...
      { command: ['gh', 'pr', 'view', '1042'], stdout: JSON.stringify(data), data },
  ...
  it('supports branch.compare with explicit base and head refs', () => {
    const result = runGithub(['branch.compare', '--base', 'main', '--head', 'stream/os', '--dry-run']);
    expect(result.ok).toBe(true);
    expect(result.operation).toBe('branch.compare');
    expect(result.command).toEqual([
      'gh',
      'api',
      'repos/consuelohq/opensaas/compare/main...stream%2Fos',
    ]);
  });
