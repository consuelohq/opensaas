...
describe('media.clip.search YouTube discovery', () => {
  it('uses fixture-driven search normalization with no live network in unit tests', async () => {
    ...
    const candidates = normalize({ items: [{ id: { videoId: 'abc123' }, snippet: { title: 'Fixture dunk breakdown', channelTitle: 'Sports Lab', publishedAt: '2026-06-20T00:00:00Z' }, statistics: { viewCount: '250000' }, contentDetails: { duration: 'PT42S' } }] });
  ...
  it('scores clips by views, velocity, recency, sport, duration, and breakdown potential', async () => {
    const module = await importMediaModule('scripts/lib/media/youtube.ts');
  ...
  it('requires yt-dlp only for media-youtube, not media-core', async () => {
    const module = await importMediaModule('scripts/lib/media/youtube.ts');
