...
describe('media.timeline.v1 contract', () => {
  it('accepts a structured vertical sports-science timeline', async () => {
  ...
  it('rejects invalid timestamps, missing assets, missing provenance, and unbounded overlays', async () => {
    const module = await importMediaModule('scripts/lib/media/schema.ts');
    const schema = getExport(module, 'MediaTimelineSchema');
  ...
  it('requires analysis-backed sports-science overlays instead of prose-only vibes', async () => {
    const module = await importMediaModule('scripts/lib/media/schema.ts');
    const schema = getExport(module, 'MediaTimelineSchema');
