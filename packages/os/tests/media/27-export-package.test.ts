...
import { expectFunctionExport, expectSchemaAccepts, getExport, importMediaModule } from './helpers';

describe('media.export package', () => {
  it('exports deterministic platform packages for Shorts, TikTok, Reels, and later longform', async () => {
  ...
  it('requires mp4, thumbnail, captions, title notes, provenance, rights notes, and render result', async () => {
    ...
    expectSchemaAccepts(schema, {
      ...
      files: { mp4: 'renders/final.mp4', thumbnail: 'renders/thumb.png', captions: 'captions/en.vtt', notes: 'notes/title.md', renderResult: 'render-result.json' },
      provenance: { sourceAssetId: 'asset_fixture_001', rightsStatus: 'needs-review' },
      deterministic: true,
    });
  });
});
