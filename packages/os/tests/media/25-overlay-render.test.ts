...
describe('media.overlay.render', () => {
  it('should satisfy media contract when it uses declarative overlay JSON and rejects arbitrary code execution', async () => {
  ...
  it('should satisfy media contract when it can emit transparent layers or PNG sequences for compose to consume', async () => {
    const module = await importMediaModule('scripts/lib/media/overlays.ts');
