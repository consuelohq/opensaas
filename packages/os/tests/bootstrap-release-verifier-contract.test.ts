...
describe('hosted bootstrap release verifier', () => {
  it('should accept library-canonical signed payloads when key ordering and undefined values differ', async () => {
    ...
    const verifierSource = bootstrap.slice(
      sourceStart + heredocStart.length,
      sourceEnd,
    );
    ...
    const payload: ChannelManifestPayload & { ignored?: undefined } = {
      ...
      platforms: [{
        ...
        cloudflareObjectKey: `bundles/${bundleId}/runtime.tar.gz`,
        bundleId,
        archiveDigest,
    ...
    const server = createServer((request, response) => {
      ...
      if (request.url === `/bundles/${bundleId}/runtime.tar.gz`) {
        response.writeHead(200, {
    ...
    try {
      ...
      await execFileAsync('bun', [verifierPath], {
        env: {
          ...
          CONSUELO_RELEASE_PUBLIC_KEYS_BASE64: Buffer.from(
            JSON.stringify({ [keyId]: signer.publicKeyPem }),
          ).toString('base64'),
      ...
      expect(
        readFileSync(join(stageDirectory, 'runtime.tar.gz')),
      ).toEqual(archiveBytes);
      expect(
        readFileSync(join(stageDirectory, 'bundle-id'), 'utf8'),
      ).toBe(`${bundleId}\n`);
    } finally {
