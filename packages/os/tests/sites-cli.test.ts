...
describe('Sites CLI', () => {
  ...
  it('materializes Nodes as the durable workspace root on every Sites refresh', () => {
    const first = runSitesCommand(['refresh', '--json']);
    const firstHtml = readFileSync(first.indexPath, 'utf8');

    expect(firstHtml).toContain('<title>Nodes - Consuelo OS</title>');
    expect(firstHtml).toContain('data-workspace-shell');
    expect(firstHtml).toContain('data-workspace-route-trigger');
    expect(firstHtml).toContain('aria-current="page" href="/nodes"');
    expect(firstHtml).toContain('/gateway/nodes/snapshot');
  ...
  it('keeps the workspace root on Nodes regardless of the authenticated workspace host', () => {
    writeFileSync(
