...
export function legacyLifecycleBootstrapCommand(channel: string): string {
  const script = [
    ...
    'return value;',
    '};',
    'const managedCloudReleaseUrlIsTrusted=(url)=>url.protocol==="https:"&&url.hostname==="storage.googleapis.com"&&!url.username&&!url.password&&!url.port&&!url.search&&!url.hash&&url.pathname.split("/").filter(Boolean).length>0;',
    'let releaseBaseUrl=process.env.CONSUELO_RELEASE_BASE_URL?.trim()||defaultReleaseBaseUrl;',
    'let releaseKeys;',
    'let gcpMetadataAuth=process.env.CONSUELO_RELEASE_GCP_METADATA_AUTH==="1";',
    ...
    'return value;',
    '};',
    'releaseBaseUrl=assignment("CONSUELO_RELEASE_BASE_URL");',
    'const managedCloudReleaseUrl=new URL(releaseBaseUrl);',
    'if(!managedCloudReleaseUrlIsTrusted(managedCloudReleaseUrl))throw new Error("managed cloud release origin is not trusted");',
    ...
    'if(gcpMetadataAuth&&!managedCloudReleaseUrlIsTrusted(releaseUrl))throw new Error("managed cloud release origin is not trusted");',
