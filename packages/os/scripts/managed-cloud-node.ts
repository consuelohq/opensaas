...
const usage = (): string =>
  [
    'usage: bun ./scripts/managed-cloud-node.ts <plan|apply> --project <project-id> --billing-account <billing-account-id> [--region <region>] [--budget-usd <amount>] [--json] [--quiet]',
    '       bun ./scripts/managed-cloud-node.ts <node-plan|node-apply> --config <node-config.json> [--json] [--quiet]',
    '',
...
export const parseManagedCloudNodeArgs = (
  ...
  if (command === 'node-plan' || command === 'node-apply') {
    ...
    return { command, configPath, json, quiet };
  }
