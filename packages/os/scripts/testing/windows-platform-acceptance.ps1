...
'@
  ...
  & $ownedBun $platformCli install-service --home $home --bun $ownedBun --service-host $serviceHost --json
  ...
  $status = & $ownedBun $platformCli status --home $home --bun $ownedBun --service-host $serviceHost --json | ConvertFrom-Json
  if ($status.state -ne 'running') { throw "Unexpected Windows service state: $($status.state)" }
  & $ownedBun $platformCli restart --home $home --bun $ownedBun --service-host $serviceHost --json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw 'Native Windows service restart failed.' }
  $diagnostics = & $ownedBun $platformCli diagnostics --home $home --bun $ownedBun --service-host $serviceHost --json | ConvertFrom-Json
  if ($diagnostics.bunExecutable -ne $ownedBun) { throw 'Diagnostics did not report the persisted Bun path.' }

  $acl = (& icacls.exe $home 2>&1 | Out-String)
  if ($acl -notmatch 'ConsueloOS') { throw 'Consuelo service ACL entry is missing.' }

  & $ownedBun $platformCli uninstall-service --home $home --bun $ownedBun --service-host $serviceHost --dry-run --json | Out-Null
  ...
  & $ownedBun $platformCli uninstall-service --home $home --bun $ownedBun --service-host $serviceHost --json | Out-Null
  ...
  & $ownedBun $platformCli install-service --home $home --bun $ownedBun --service-host $serviceHost --json | Out-Null
  ...
  if ($installed -and (Test-Path -LiteralPath $ownedBun)) {
    & $ownedBun $platformCli uninstall-service --home $home --bun $ownedBun --service-host $serviceHost --json 2>$null | Out-Null
  }
