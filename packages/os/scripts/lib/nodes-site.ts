  ...
  return `
      ...
      <dialog id="add-node-dialog" class="node-dialog" aria-labelledby="add-node-title">
        <div class="dialog-shell">
          ...
          <div class="actions"><button id="create-cloud-node-button" class="primary-button provisioning-button" type="button" disabled>Continue to checkout</button><button id="add-node-cancel" type="button">Cancel</button></div>
...
export function nodesClientScript(): string {
  return String.raw`
    ...
    function updateCreateButton() { const button = byId('create-cloud-node-button'); if (!(button instanceof HTMLButtonElement)) return; const ready = Boolean(selectedQuote()) && !currentProvisioningJobId; button.disabled = !ready; button.textContent = currentProvisioningJobId ? 'Creating cloud node…' : 'Continue to checkout'; }
    ...
    async function createCloudNode() { const csrf = csrfToken(); const quote = selectedQuote(); if (!csrf || !quote) { setText('pricing-status', 'A current monthly quote is required before creating a node.'); return; } const button = byId('create-cloud-node-button'); if (button instanceof HTMLButtonElement) button.disabled = true; currentProvisioningKey = currentProvisioningKey || (globalThis.crypto && typeof globalThis.crypto.randomUUID === 'function' ? globalThis.crypto.randomUUID() : 'cloud-' + Date.now() + '-' + Math.random().toString(16).slice(2)); setHidden('provisioning-progress', false); setText('provisioning-phase', 'Secure checkout'); setText('provisioning-detail', 'Opening Stripe to confirm the shown monthly price…'); let failureMessage = 'Nothing was charged or provisioned. Check your connection and try again.'; try { const response = await fetch('/gateway/nodes/provision', { method: 'POST', credentials: 'same-origin', headers: { accept: 'application/json', 'content-type': 'application/json', 'x-consuelo-csrf-token': csrf }, body: JSON.stringify({ planId: quote.plan.id, region: quote.region.id, pricingVersion: quote.pricingVersion, idempotencyKey: currentProvisioningKey }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) { failureMessage = payload && payload.error && payload.error.message ? payload.error.message : failureMessage; throw new Error('checkout failed'); } const checkout = payload && payload.checkout; if (!checkout || typeof checkout.url !== 'string' || !checkout.url.startsWith('https://')) throw new Error('missing checkout'); setText('provisioning-phase', 'Opening secure checkout'); setText('provisioning-detail', 'You will return here after checkout. Provisioning starts only after Stripe confirms payment.'); window.location.assign(checkout.url); } catch { currentProvisioningJobId = null; currentProvisioningKey = null; setText('provisioning-phase', 'Cloud node was not created'); setText('provisioning-detail', failureMessage); updateCreateButton(); } }

    function applyCheckoutReturnFeedback() { const state = new URLSearchParams(window.location.search).get('checkout'); if (state === 'success') { setText('node-feedback', 'Checkout complete. Payment confirmation and provisioning will continue automatically.'); } else if (state === 'cancelled') { setText('node-feedback', 'Checkout cancelled. No cloud node was created.'); } if (state) window.history.replaceState(null, '', '/nodes'); }
