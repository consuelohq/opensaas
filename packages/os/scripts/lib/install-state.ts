...
function materializeLifecycleCommand(
  ...
  const source = [
    ...
    'if [ ! -f "$LIFECYCLE_SCRIPT" ] && [ -n "${CONSUELO_OS_PACKAGE_ROOT:-}" ]; then',
    '  LIFECYCLE_SCRIPT="$CONSUELO_OS_PACKAGE_ROOT/scripts/lifecycle.ts"',
    'fi',
    'if [ ! -f "$LIFECYCLE_SCRIPT" ]; then',
    '  echo "OS lifecycle runtime is not installed. Run: curl -fsSL https://install.consuelohq.com/os | bash" >&2',
    '  exit 1',
    'fi',
    'exec bun "$LIFECYCLE_SCRIPT" --home "$OS_HOME" "$@"',
    '',
  ].join('\n');
