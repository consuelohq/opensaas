...
export type WorkspaceEdgePublishResult = { status: 'succeeded'; workspaceId: string; workspaceSlug: string; workspaceHost: string; siteId: string; versionId: string; snapshotKey: string; snapshotPath: string; verifyUrl: string; logPath: string; httpStatus: number; cacheAuthority: string | null; sitesCache: string | null };
...
export function createWorkspaceEdgeSnapshotPlan(input: PublishInput): WorkspaceEdgeSnapshotPlan {
  ...
  const record = { workspaceId: input.workspaceId, workspaceSlug: input.workspaceSlug, hostname: workspaceHost, baseDomain: baseDomain(workspaceHost), provider: 'cloudflare', owner: 'consuelo-os-cloud', status: 'active', routes: [{ surface: 'sites', pathPrefix: '/', auth: 'public', status: 'active', target: { kind: 'site-snapshot', siteId, versionId: version, manifestKey: snapshotKey, contentType, cachePolicy: 'static-shell' } }], updatedAt: input.now ?? new Date().toISOString() };
