...
import { readLocalStreamDecisions } from '../stream-memory';
...
export function printStreamContext(result: StreamContextResult, json: boolean): void {
  ...
  if (result.decisions.length === 0) {
    ...
    for (const decision of result.decisions) {
      writeLine(`  - ${decision.title}  (${decision.date})`);
    }
  }
  writeLine(
    `  tip: bun run memory -- save "${result.area}: new decision" --text --category stream-decision`,
  );
  writeLine('');
