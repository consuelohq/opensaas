...
export function renderCaddyGatewayConfig(input: {
  ...
  return `{\n  admin off\n  auto_https off\n}\n\nhttp://:${ingressPort} {\n  bind 127.0.0.1\n  @workspace_host host ${input.workspaceHost}\n  handle @workspace_host {\n    encode zstd gzip\n    request_body {\n      max_size 4MB\n    }\n    header {\n      -Server\n      X-Content-Type-Options \"nosniff\"\n      Referrer-Policy \"no-referrer\"\n      Permissions-Policy \"camera=(), microphone=(), geolocation=()\"\n    }\n    reverse_proxy ${input.upstream.host}:${input.upstream.port} {\n      # Preserve Consuelo edge auth headers so the local OS gateway can verify\n      # device-authority / ChatGPT-proxied requests. Stripping them forced a second\n      # OAuth introspect hop and broke signed connector traffic.\n      transport http {\n        dial_timeout 5s\n        response_header_timeout 15s\n        read_timeout 60s\n        write_timeout 60s\n      }\n    }\n  }\n  respond \"Misdirected Request\" 421\n}\n`;
}
