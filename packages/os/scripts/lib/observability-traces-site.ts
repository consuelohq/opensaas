  ...
  <script>
    (function mountObservabilityTraces() {
      ...
      const open = () => {
        ...
        render();
        if (window.gsap) window.gsap.fromTo('.trace-shell', { autoAlpha: 0, y: 18, scale: 0.98 }, { autoAlpha: 1, y: 0, scale: 1, duration: 0.22, ease: 'power2.out' });
      };
      ...
      function connectEvents() {
        ...
        const source = new EventSource(eventsUrl + '?cursor=' + encodeURIComponent(state.cursor || 'cur_000'));
        const handle = (event) => {
