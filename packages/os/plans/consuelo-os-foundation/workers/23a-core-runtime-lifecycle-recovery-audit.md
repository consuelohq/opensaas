# Worker 23a: Core Runtime, Lifecycle, and Recovery Audit
...
Determine whether the installed runtime, lifecycle authority, managed-component model, recovery behavior, and steering boundary satisfy the approved product contract under normal, interrupted, stale, and adversarial conditions.
...
Prefix finding IDs and GitHub summaries with `23A`.
...
Review install-state authority, lifecycle locking and journaling, install/update/status/restart/repair/rollback/uninstall/dev-reset state machines, atomic activation, automatic rollback, retention and pruning, managed-component provenance and merge classification, user-owned content preservation, update notifications, diagnostics/redaction, installed skills, runtime/node identity in steering, and exclusion of `decision.md`. Trace all adapters back to one lifecycle authority and identify any second updater, restart authority, or mutable runtime source.

## Required evidence

Require focused and broad lifecycle suites, clean-home and N-1 update evidence, interruption/failure-injection coverage, retention/uninstall assertions, steering and skills registry evidence, runtime-bundle fixture evidence, and registered clean-host CI. Inspect state files and failure paths rather than trusting happy-path command output.

## Adversarial review journeys

1. Interrupt update before download, after verification, during migration, after activation, and during health acceptance.
2. Start concurrent lifecycle mutations, recover stale locks, and verify replacement workers cannot overlap.
3. Fail health acceptance and prove automatic rollback restores a known-good runtime and services.
4. Repair a damaged runtime without onboarding or destructive reset.
5. Uninstall with and without user-content removal and verify ownership boundaries.
6. Preserve modified managed components, surface conflicts, and prune only unneeded releases.
7. Verify steering remains compact, redacted, notification-aware, and excludes `decision.md`.
...
Write the durable domain report to `packages/os/plans/consuelo-os-foundation/reviews/final/23a-report.md`. The report summarizes evidence and links to GitHub; it does not replace GitHub comments.

## Acceptance gates

- One canonical lifecycle engine owns all mutations.
- Interrupted and failed operations have deterministic recovery with no split-brain runtime state.
- Current, previous, pinned, and unresolved merge bases are retained exactly as specified.
- User content, auth, workspace identity, and steering survive ordinary update/repair.
- No secret, local user path, or unbounded diagnostic crosses CLI, IPC, logs, or steering.
