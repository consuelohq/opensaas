...
- `packages/os/scripts/lib/workspace-cloudflare-provisioning.ts` builds managed OS MCP custom-rule expressions for the OS workspace hostname class under `*.consuelohq.com`, excluding only the opaque `c-<32-hex>.consuelohq.com` connector-origin class, `workspace.consuelohq.com`, and reserved platform hosts.
