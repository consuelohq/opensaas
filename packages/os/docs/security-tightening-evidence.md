...
- `packages/os/scripts/lib/workspace-cloudflare-provisioning.ts` builds managed OS MCP custom-rule expressions for workspace hostnames under `*.consuelohq.com`, excluding the opaque `c-<32-hex>.consuelohq.com` connector-origin class and explicit non-workspace platform MCP surfaces. Customer-claim reservation is not used as an ingress-security exclusion, so a platform-owned workspace such as `internal.consuelohq.com` remains covered by the workspace MCP source-IP policy.
...
- `.github/workflows/consuelo-production-release.yaml` exposes full managed MCP ingress reconciliation only as an explicit `workflow_dispatch` action. The older connector-origin migration remains separate because it only rewrites the connector-origin expression fragment and is not sufficient to reconcile hostname coverage such as the `internal.consuelohq.com` workspace case.
