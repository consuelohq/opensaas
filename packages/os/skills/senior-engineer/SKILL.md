...
Use no-session code.run for multi-step read/investigation before a task exists.
Use task-scoped code.run for semantic workflows that compose multiple typed tools inside a task.
Use exact task-scoped file tools for file work: fs.read, fs.search, fs.list, fs.patch, fs.write, and fs.trash.
...
Do not wrap a single obvious typed call in code.run. Use direct os.call for one known operation unless code.run is adding summarization, branching, or composition value.
