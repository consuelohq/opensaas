Research Ingest
Use the workspace research:ingest tool to turn research sources into durable memory before analysis.
...
Successful ingest automatically saves a self-contained text bundle to memory. Do not add an extra approval step for memory save.
...
Use memoryTitle when Ko gives a clear title or when a better durable title is obvious:
...
await os.call({
  ...
  input: {
    source: "<source>",
    question: "<question>",
    memoryTitle: "Research Bundle: <clear title>"
  },
  timeout: 600,
})
Do not use noMemorySave in normal user-facing work. It is only for tests/debugging or when Ko explicitly asks not to save.
...
memory-bundle.md — full text bundle saved to memory
summary.json and raw logs for debugging
optional slides/ only when visual extraction is enabled
The memory entry must contain the full text of:
...
Let research.ingest autosave the full text bundle to memory.
Use the returned packet/memory information as the grounding source for the next response.
...
Default run artifacts are under the operating system temp directory, so local frames/slides can be temporary. This is okay because the text memory bundle is durable.

Use keep: true only when Ko wants durable local files in addition to saved memory.
...
the fact that the bundle was saved to memory
...
Give each one a clear memoryTitle.
