Core rule
Use built-in web search for discovery. Use OS browser tools for known URLs, rendered-page extraction, authenticated pages, UI verification, screenshots, and interaction replay.
...
os.call({ tool: "browser.click", input: { ref: "@e1" } })
os.call({ tool: "browser.fill", input: { ref: "@e1", text: "<text>" } })
os.call({ tool: "browser.get", input: { target: "title" } })
os.call({ tool: "browser.find", input: { by: "role", value: "button", action: "click", name: "Submit" } })
os.call({ tool: "browser.wait", input: { load: "networkidle" } })
os.call({ tool: "browser.download", input: { ref: "@e1", path: "/tmp/download.bin" } })
os.call({ tool: "browser.tabs", input: { action: "list" } })
os.call({ tool: "browser.cookies", input: { action: "list" } })
os.call({ tool: "browser.network", input: { args: ["requests"] } })
...
os.call({ tool: "browser.test", input: { url: "<url>", full: true }, timeout: 300000 })
...
os.call({ tool: "browser.screenshot", input: { name: "after-extraction", full: true }, timeout: 300000 })
...
os.call({ tool: "browser.raw", input: { args: ["tab", "list"] }, timeout: 300000 })
