...
const packageRoot = 'packages/lead-connector';
const outputRoot = `${packageRoot}/dist/embed-app`;
...
describe('LeadConnector embed build', () => {
  it('content-versions every application asset reference with its exact digest', () => {
    const build = Bun.spawnSync({
      cmd: ['bun', `${packageRoot}/scripts/build-embed.ts`],
      stdout: 'pipe',
      stderr: 'pipe',
    });
    expect(build.exitCode).toBe(0);

    const html = readFileSync(`${outputRoot}/index.html`, 'utf8');
    const cssVersion = digestPrefix(`${outputRoot}/main.css`);
    const jsVersion = digestPrefix(`${outputRoot}/main.js`);
