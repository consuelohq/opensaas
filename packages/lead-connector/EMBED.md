...
- `consuelo-lead-connector-click-to-call.marketplace-loader.html` is the preferred one-time production bootstrap. It is a small inline bootstrap that idempotently loads the launcher stylesheet and script from the dialer-owned Cloudflare custom domain `https://calls.consuelohq.com`. The Worker also owns the exact `calls.consuelohq.com/*` route, which is more specific than the OS workspace `*.consuelohq.com/*` fallback and therefore keeps dialer traffic out of the workspace protection layer. `workers.dev` and preview URLs are disabled for this Worker. Once that bootstrap is installed and verified in Marketplace, normal releases update the Cloudflare-hosted launcher assets and do not require Marketplace browser or developer-session credentials.
...
- The Marketplace sidebar/custom-page module points to `https://calls.consuelohq.com/admin` and grants microphone permission where HighLevel exposes that setting.
