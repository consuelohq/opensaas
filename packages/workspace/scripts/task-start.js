...
function printHelp() {
  ...
  writeStdout('  missing streams must be created first with stream.create');
...
async function ensureRemoteStreamBranch({ token, repository, streamBranch }) {
  try {
    const streamRef = await getBranchRef({ token, repository, branch: streamBranch });
    if (!streamRef) {
      throw new Error(
        `remote stream ${streamBranch} does not exist. create it first with ` +
        `workspace.call({ tool: "stream.create", input: { area: "${streamBranch.replace(/^stream\//, '')}" } })`,
      );
    }
    return { streamRef };
  } catch (error) {
