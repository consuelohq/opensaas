...
function printHelp() {
  ...
  writeStdout('  --approved             Ko-approved override for invalid/missing verify stamp; requires --reason');
  ...
  if (verifyMismatch) {
    if (!args.approved) {
      throw new Error(
        ...
        'run: bun run verify\n' +
        'approved override requires explicit Ko approval: bun run task:push -- --approved --reason "Ko approved: ..." --message "fix(area): summary" --changed',
      );
    }
    ...
    writeStderr(`APPROVED PUSH OVERRIDE USED: ${args.reason}`);
  } else if (args.approved) {
    writeStderr('warning: --approved provided but verify stamp is publish-valid; override not needed');
  }
