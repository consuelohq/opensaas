...
function getReviewRunDir(repoRoot) {
  return gitPath(repoRoot, 'opensaas-review-runs');
}
...
function normalizeArgs(args = {}) {
  return {
    ...
    summaryJson: Boolean(args.summaryJson),
    noTests: Boolean(args.noTests),
    strict: Boolean(args.strict),
    quiet: Boolean(args.quiet),
  };
}

function makeReviewRunIdentity({ repoRoot, branch, base, verificationState, args }) {
  const payload = {
    schema: 'review-run-identity.v1',
...
function pathsForIdentity(repoRoot, identity) {
  const dir = path.join(getReviewRunDir(repoRoot), identity.key);
  return {
    dir,
...
function waitForExistingRun(paths, waitMs) {
  ...
  const record = readActiveRecord(paths);
  const error = new Error(`review run still running after ${waitMs}ms`);
  error.code = 'REVIEW_RUN_STILL_RUNNING';
  error.record = record;
  throw error;
}

function beginReviewRun(repoRoot, identity, options = {}) {
  const waitMs = Number(options.waitMs || process.env.WORKSPACE_REVIEW_RUN_WAIT_MS || DEFAULT_WAIT_MS);
  ...
  while (true) {
    try {
      ...
      // Existing lock was stale or orphaned. Loop and try to acquire it.
    }
  }
}

function finishReviewRun(run, result) {
