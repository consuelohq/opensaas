...
export type SubagentRunSummary = {
  ...
  filesEdited: string[];
  toolsCalled: string[];
  traceEvents: Array<{ tool: string; status: string; input?: string; output?: string; traceId?: string }>;
};
...
function subagentToolResult(
  ...
  const data: SubagentData = {
    ...
    ...(input.subagentTrace ? { subagentTrace: input.subagentTrace } : {}),
...
function compactSubagentOutput(input: {
  ...
  stderr: string;
  audit?: SubagentData['audit'];
}): Pick<SubagentData, 'stdout' | 'stderr' | 'finalMessage' | 'summary' | 'rawLogPath' | 'stdoutLogPath' | 'stderrLogPath' | 'stdoutChars' | 'stderrChars' | 'usage' | 'subagentTrace'> {
  const parsed = parseSubagentOutput(input.provider, input.stdout);
  const logs = persistSubagentLogs(input);
  const subagentTrace = persistSubagentTraceEvents({
    provider: input.provider,
    cwd: input.cwd,
  ...
  return {
    ...
    ...(subagentTrace && subagentTrace.eventCount > 0 ? { subagentTrace } : {}),
  };
}
...
function persistSubagentTraceEvents(input: {
  provider: SubagentProvider;
  cwd: string;
  traceId: string;
  stdout: string;
  ...
  branch?: string;
  stdoutLogPath?: string;
}): SubagentTraceSummary | undefined {
  const events = parseSubagentTraceEvents(input.provider, input.stdout);
  const summary = summarizeSubagentTraceEvents(events);
  ...
  try {
    ...
    try {
      ...
      for (const [index, event] of events.entries()) {
        const childTraceId = `${input.traceId}:subagent:${String(index + 1).padStart(4, '0')}`;
        const inputJson = {
          provider: input.provider,
        ...
        insert.run(
          `${input.traceId}:subagent:${stableHash(`${index}:${event.itemId || event.tool}:${event.eventType}`)}`,
          now,
          childTraceId,
          input.traceId,
          'subagent',
    ...
    // Subagent trace ingestion must not fail the subagent result.
  }
  return summary;
}

export function parseSubagentTraceEvents(provider: SubagentProvider, stdout: string): SubagentTraceEvent[] {
  if (provider !== 'codex') return [];
  ...
  for (const line of stdout.split(/\r?\n/)) {
    ...
    if (event.type === 'item.completed') {
      ...
      if (item.type === 'mcp_tool_call') {
        ...
        const tool = isGetSteering ? 'codex.get_steering' : facadeTool ? `codex.${facadeTool}` : `codex.${server}.${mcpTool}`;
...
function isWorkspaceMcpTraceEvent(event: SubagentTraceEvent): boolean {
  return event.eventType === 'mcp_tool_call'
    && (Boolean(event.facadeTool) || event.tool === 'codex.get_steering' || event.tool.startsWith('codex.workspace.'));
}

function summarizeSubagentTraceEvents(events: SubagentTraceEvent[]): SubagentTraceSummary {
  return {
    ...
    getSteeringCount: events.filter((event) => event.tool === 'codex.get_steering').length,
