...
function writeArchiveServer(ip: string): void {
  ...
  const traceHistoryModuleUrl = pathToFileURL(
    path.join(
      WORKSPACE_ROOT,
      'scripts/trace-site-inspector/archive-history.ts',
    ),
  ).href;
  const traceDbResolverModuleUrl = pathToFileURL(
    path.join(
      REPO_ROOT,
  ...
  const lines = [
    'import { createArchiveTraceHistoryResponse, enrichTracePayloadWithBatchResults } from ' +
      JSON.stringify(traceHistoryModuleUrl) +
      ';',
    'import { resolveTraceDbPath } from ' +
      JSON.stringify(traceDbResolverModuleUrl) +
      ';',
    ...
    "async function liveTracesResponse(filePath){ try { const payload = JSON.parse(await Bun.file(filePath).text()); const enriched = enrichTracePayloadWithBatchResults(payload, latestTraceDb()); return new Response(JSON.stringify(enriched), { headers: h(\"application/json; charset=utf-8\") }); } catch { return new Response(Bun.file(filePath), { headers: h(\"application/json; charset=utf-8\") }); } }",
