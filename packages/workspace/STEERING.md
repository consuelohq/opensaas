...
| `cat > file <<EOF ... EOF` | `tmp` + `fs.write` with `contentFile`; `fs.patch` with `contentFile`; or `fs.applyPatch` with `patchFile` for marker/diff patches | Heredocs are fragile and often safety-filtered. |
...
- Use `fs.applyPatch` for anchored marker/diff patches, especially multi-file edits and add/move/delete operations.
