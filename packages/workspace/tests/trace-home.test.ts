...
import { initialTraceHomeState, reduceTraceHomeState } from '../scripts/trace-home/tui/state';
import type { TraceHomeRow } from '../scripts/trace-home/types';

function row(overrides: Partial<TraceHomeRow>): TraceHomeRow {
  return { rownum: 1, record_id: 'record-1', ts: '2026-06-01T20:00:33.000Z', trace_id: 'trc_1', tool: 'status', task_session: 'tsk_1', branch: 'task/workspace/example', worktree: '.task/workspace/example', status: 'ok', code: 'OK', exit_code: 0, duration_ms: 100, input_tokens: 10, output_tokens: 5, total_tokens: 15, input_json: '{}', resolved_input_json: '{}', result_json: JSON.stringify({ ok: true, code: 'OK', message: 'command completed' }), stderr: '', ...overrides };
}
const broad = ['r','m',' ','-','r','f'].join('');
const wrapper = 'execFile' + "Sync('gh', args, { encoding: 'utf8'";
const fixtureRows: TraceHomeRow[] = [
  row({ rownum: 1, record_id: 'batch-1', trace_id: 'trc_batch', tool: 'batch', branch: 'task/os/os-local-testing-readiness', duration_ms: 1200, total_tokens: 538, resolved_input_json: JSON.stringify([{ tool: 'fs.trash', input: { path: '.task/a' } }, { tool: 'fs.trash', input: { path: '.task/b' } }]), result_json: JSON.stringify({ ok: true, code: 'OK', data: { results: [{ ok: true, code: 'OK', tool: 'fs.trash', durationMs: 220, totalTokens: 89, detail: 'changed .task/a' }, { ok: true, code: 'OK', tool: 'fs.trash', durationMs: 180, totalTokens: 93, detail: 'changed .task/b' }] } }) }),
  row({ rownum: 2, record_id: 'task-exec-good', trace_id: 'trc_good', tool: 'task.exec', duration_ms: 140, total_tokens: 3400, resolved_input_json: JSON.stringify({ command: ['bun', 'test', 'packages/workspace/tests/trace-home.test.ts'] }) }),
  row({ rownum: 3, record_id: 'task-call-suspect', trace_id: 'trc_suspect', tool: 'task.call', status: 'failed', code: 'COMMAND_FAILED', exit_code: 1, duration_ms: 1000, total_tokens: 359, resolved_input_json: JSON.stringify({ command: ['bash', '-lc', "sed -n '1,260p' packages/workspace/scripts/trace-home/model.ts"] }), stderr: wrapper + '\njob is still in progress; logs unavailable', result_json: JSON.stringify({ ok: false, code: 'COMMAND_FAILED', message: 'command failed' }) }),
  row({ rownum: 4, record_id: 'task-exec-bad', trace_id: 'trc_bad', tool: 'task.exec', duration_ms: 900, total_tokens: 1200, resolved_input_json: JSON.stringify({ command: ['bash', '-lc', `${broad} .task/tmp`] }) }),
  row({ rownum: 5, record_id: 'code-run-1', trace_id: 'trc_code', tool: 'code.run', duration_ms: 8470, total_tokens: 42900, result_json: JSON.stringify({ ok: true, code: 'OK', data: { operations: [{ ok: true, code: 'OK', tool: 'fs.read', durationMs: 80, totalTokens: 212, detail: 'README.md' }, { ok: true, code: 'OK', tool: 'fs.search', durationMs: 1210, totalTokens: 3200, detail: 'pattern: trace:home' }, { ok: true, code: 'OK', tool: 'git.diff', durationMs: 410, totalTokens: 1400, detail: 'changes' }] } }) }),
];
...
describe('trace home', () => {
  ...
  test('keyboard reducer covers pause, selection, failed-only, group, and help', () => {
    ...
    expect(state).toMatchObject({ paused: true, selectedIndex: 0, failedOnly: true, group: 'branch', help: true });
  });
});
