...
function createFakeGh(): { dir: string; logFile: string } {
  const dir = mkdtempSync(join(tmpdir(), 'github-pr-reviews-'));
...
if (args[0] === 'pr' && args[1] === 'view') {
  out({
    number: 1,
    title: 'Review collector PR',
    headRefName: 'task/review-collector',
    baseRefName: 'stream/workspace-agents',
    state: 'OPEN',
    files: [{ path: 'packages/workspace/scripts/github.js' }],
  ...
  out([[
    {
      id: 100,
      path: 'packages/workspace/scripts/github.js',
      line: 12,
