...
const { createWorkspaceHookDispatcher } = require('./dispatcher.js');

const DEFAULT_MANIFEST_PATH = path.join(__dirname, '..', 'tooling', 'tool-manifest.json');
...
function loadWorkflowBundles(options = {}) {
  ...
  if (!bundles || bundles.kind !== 'consuelo-workspace-workflow-bundles' || !Array.isArray(bundles.workflows)) {
...
function createWorkflowIntentRuntime(options = {}) {
  ...
  const dispatcher = createWorkspaceHookDispatcher({
    manifest,
