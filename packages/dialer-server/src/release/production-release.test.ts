...
describe('dialer production release safety', () => {
  it('selects the new Railway deployment instead of racing the previous latest', () => {
    ...
    expect(
      selectNewRailwayDeployment(deployments, 'previous-deployment'),
    ).toEqual({
      id: 'new-deployment',
      status: 'DEPLOYING',
    });
  ...
  it('smokes only non-mutating health/auth/signature boundaries', async () => {
    ...
    const fetcher = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response('{}', {
        status: expected.get(`${method} ${url.pathname}`) ?? 599,
      });
    };
    ...
    expect(requests.every(({ authorization }) => authorization === null)).toBe(
      true,
    );
  ...
  it('requires immutable one-time Marketplace bootstrap evidence', () => {
    expect(() =>
      buildDialerReleaseManifest({
        gitSha: 'abc123',
        railway: { deploymentId: 'railway-1', status: 'SUCCESS' },
    ...
    ).toThrow('launcher bootstrap SHA-256');
  });
  ...
  it('rolls Railway back to one explicit deployment id through the public API', async () => {
    ...
    const fetcher = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify({ data: { deploymentRollback: { id: 'new-deploy' } } }),
        {
          status: 200,
