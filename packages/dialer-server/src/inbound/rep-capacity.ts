...
// Internal composition seam: caller must hold withInboundTransaction's tenant lock.
export const executeRepCapacityOnClient = async (
  client: PoolClient,
  raw: RepCapacityInput,
  options: { clock?: () => string } = {},
): Promise<RepCapacityResult> => {
  ...
  try {
    ...
    if (previous?.owner && ['accept', 'dispatch'].includes(input.action.type)) {
      ...
      if (
        !request ||
        !['created', 'queued', 'offering', 'bridging'].includes(request.state)
      )
        throw new InboundPersistenceError(
    ...
    if (input.action.type === 'dispatch')
      await claimRepCapacityCommand(client, state, input.action.commandId);
    if (
      previous?.owner &&
...
export const createPostgresRepCapacity = (
  ...
  const execute = (raw: RepCapacityInput): Promise<RepCapacityResult> =>
    withInboundTransaction(pool, raw.workspaceId, (client) =>
      executeRepCapacityOnClient(client, raw, options),
    );
  const read = async (
    workspaceId: string,
    capacityId: string,
