...
const checkService = async (
  ...
  full: ReturnType<typeof loadFullConfig>,
): Promise<CheckResult> => {
    ...
    switch (name) {
      ...
      case 'twilio': {
        ...
        return {
          ...
          status: res.ok ? 'healthy' : 'unhealthy',
          latency: Date.now() - start,
          message: res.ok ? undefined : 'invalid credentials',
        };
      }
      case 'ai': {
        ...
        return {
          ...
          status: res.ok ? 'healthy' : 'unhealthy',
          latency: Date.now() - start,
          message: res.ok ? undefined : 'invalid api key',
        };
      }
    }
  } catch (err: unknown) {
...
const getResources = (): {
  ...
  try {
    disk = execSync('df -h . 2>/dev/null | tail -1', {
      encoding: 'utf-8',
    }).trim();
  } catch (_err: unknown) {
    // disk info unavailable — intentional: optional system command
  }
...
const runStatus = async (opts: StatusOpts): Promise<void> => {
  try {
    ...
    const targets: ServiceName[] = opts.service
      ? [opts.service as ServiceName]
      : SERVICES;

    if (opts.service && !SERVICES.includes(opts.service as ServiceName)) {
      throw new Error(
        'unknown service: ' + opts.service + '. valid: ' + SERVICES.join(', '),
      );
    }

    const results = await Promise.all(
      targets.map((s) => checkService(s, config, full)),
    );
    ...
    if (!opts.service) {
      ...
      log(
        '  memory: ' +
          mb(resources.memTotal - resources.memFree) +
          ' / ' +
          mb(resources.memTotal) +
          ' (' +
          resources.memUsedPct +
          '%)',
      );
      if (resources.disk) {
...
export function registerStatus(program: Command): void {
  program
    ...
    .option(
      '--service <name>',
      'check a specific service (api, twenty, database, twilio, ai)',
    )
    ...
    .action(async (opts: StatusOpts) => {
      if (opts.watch) {
        ...
        const loop = async (): Promise<void> => {
          try {
            if (!isJson())
              log(chalk.dim('— ' + new Date().toLocaleTimeString() + ' —'));
