  ...
  async run(): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        if ((err as { failedWorkspaceMigrationBuildResult?: unknown }).failedWorkspaceMigrationBuildResult) {
          console.error(JSON.stringify((err as { failedWorkspaceMigrationBuildResult: unknown }).failedWorkspaceMigrationBuildResult, null, 2));
        }
      }
    }
