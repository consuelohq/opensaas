...
export class QueuesController {
  ...
  @Post('queues')
  async createQueue(@Req() request: AuthRequest, @Body() body: Record<string, unknown>) {
  ...
  async pauseQueue(@Req() request: AuthRequest, @Param('id') id: string) {
    ...
    const queue = await this.queuesService.updateQueueStatus(workspaceId, id, 'paused');
    if (!queue) {
      throw new NotFoundException('Queue not found');
    }
  ...
  async resumeQueue(@Req() request: AuthRequest, @Param('id') id: string) {
    ...
    const queue = await this.queuesService.updateQueueStatus(workspaceId, id, 'active');
    if (!queue) {
      throw new NotFoundException('Queue not found');
    }
  ...
  async skipQueueItem(
    ...
    return this.queuesService.skipCurrentItem(id, workspaceId, body.reason);
  }
  ...
  @HttpCode(200)
  async restartQueue(@Param('id') id: string) {
    await this.queuesService.restartQueue(id);
  ...
  async assignQueue(
    ...
    const queue = await this.queuesService.assignQueue(workspaceId, id, body.userId);
    if (!queue) {
      throw new NotFoundException('Queue not found');
    }
  ...
  async getQueueAnalytics(@Param('id') id: string) {
    return this.queuesService.queueAnalytics(id);
  }

  @Get('queues/:id/export')
  async exportQueueCsv(@Param('id') id: string, @Res() response: Response) {
    const csv = await this.queuesService.exportQueueCsv(id);
  ...
  async getContactDialerInfo(@Param('id') id: string) {
    return this.queuesService.getContactDialerInfo(id);
  }
}
