...
const initHeroMotion = (splits: SplitInstance[]) => {
  ...
  if (media) {
    timeline
      .from(media, { y: 18, autoAlpha: 0.4, duration: 0.64 }, '-=0.3')
      .call(() => runGlint(media));
  }
};
...
const initHeroTabs = (cleanups: Cleanup[]) => {
  ...
  tabButtons.forEach((button) => {
    ...
    addListener(cleanups, button, 'mouseenter', () => runGlint(button));
  });
...
const initScrollReveals = (splits: SplitInstance[]) => {
  toElements<HTMLElement>('[data-motion~="section-reveal"]').forEach((section) => {
    const targets = toElements<HTMLElement>(
      '[data-motion~="feature-row"], [data-motion~="stat-card"], [data-motion~="privacy-row"], [data-motion~="footer-item"], [data-motion~="mercury-card"]',
      section,
    );
