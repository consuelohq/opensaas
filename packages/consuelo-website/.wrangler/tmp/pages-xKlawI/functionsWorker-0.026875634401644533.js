...
async function logToSupabase(env2, table3, data) {
  await fetch(env2.SUPABASE_URL + "/rest/v1/" + table3, {
    method: "POST",
    headers: {
...
__name(logToSupabase, "logToSupabase");
async function patchSupabase(env2, table3, filter, data) {
  await fetch(env2.SUPABASE_URL + "/rest/v1/" + table3 + "?" + filter, {
    method: "PATCH",
    headers: {
      "apikey": env2.SUPABASE_KEY,
      "Authorization": "Bearer " + env2.SUPABASE_KEY,
...
__name(patchSupabase, "patchSupabase");
async function logToPosthog(env2, event, leadId, properties) {
...
async function onRequestGet(context2) {
  ...
  if (action === "open") {
    context2.waitUntil(Promise.all([
      ...
      logToPosthog(env2, "email_opened", leadId, {})
    ]));
  ...
  if (action === "click") {
    ...
    context2.waitUntil(Promise.all([
      ...
      logToPosthog(env2, "email_clicked", leadId, { url: targetUrl })
    ]));
  ...
  if (action === "unsub") {
    context2.waitUntil(Promise.all([
      ...
      patchSupabase(env2, "leads", "id=eq." + leadId, {
        status: "unsubscribed",
        updated_at: now
      }),
      logToPosthog(env2, "email_unsubscribed", leadId, {})
    ]));
...
// ../.wrangler/tmp/pages-xKlawI/functionsRoutes-0.8063145507711664.mjs
var routes = [
  {
    routePath: "/t/:action",
    mountPath: "/t",
