...
This guide provides step-by-step instructions to install and configure the Consuelo application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Install the latest stable version of Consuelo with a single command:
```bash
...
### Expose Consuelo to External Access

By default, Consuelo runs on `localhost` at port `3000`. To access it via an external domain or IP address, you need to configure the `SERVER_URL` in your `.env` file.
...
SSL (HTTPS) is required for certain browser features to work properly. While these features might work during local development (as browsers treat localhost differently), a proper SSL setup is needed when hosting Consuelo on a regular domain.
...
We strongly recommend setting up Consuelo behind a reverse proxy with SSL termination for optimal security and functionality.
