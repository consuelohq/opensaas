---
title: Consuelo Apps
description: Build and manage Consuelo customizations as code.
---
...
Apps let you build and manage Consuelo customizations **as code**. Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
- A Consuelo workspace and an API key (create one at https://app.consuelohq.com/settings/api-webhooks)
...
<Note>
**Base fields are created automatically.** When you define a custom object, Consuelo automatically adds standard fields such as `name`, `createdAt`, `updatedAt`, `createdBy`, `position`, and `deletedAt`. You don't need to define these in your `fields` array — only add your custom fields.
</Note>
...
Front components let you build custom React components that render within Consuelo's UI. Use `defineFrontComponent()` to define components with built-in validation:
...
Key points:
- Front components are React components that render in isolated contexts within Consuelo.
...
When your function runs on Consuelo, the platform injects credentials as environment variables before your code executes:

- `TWENTY_API_URL`: Base URL of the Consuelo API your app targets.
...
- Cannot connect to server: verify the API URL and that the Consuelo server is reachable.
