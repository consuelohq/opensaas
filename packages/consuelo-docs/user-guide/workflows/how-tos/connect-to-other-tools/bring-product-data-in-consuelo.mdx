---
title: Bring Product Data into Consuelo
description: Sync product catalog data from a data warehouse into your CRM on a schedule.
---

Use this pattern to keep Consuelo in sync with product data from your data warehouse (e.g., Snowflake, BigQuery, PostgreSQL).
...
5. **Upsert Record**: Create or update in Consuelo
...
export const main = async (params: {
  ...
  return {
    users: usersFormatted.map((user) => ({
      ...
      isConsuelo: toBool(user.isConsuelo),
    })),
  };
};
```
