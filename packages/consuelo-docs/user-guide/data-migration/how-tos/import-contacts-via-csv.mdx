...
description: Complete step-by-step guide to importing people/contacts into Consuelo.
---

## Overview

This guide walks you through importing your contacts (People) into Consuelo. **Import Companies first** if you want to link People to Companies.
...
- **Must be unique** — no duplicates in your file or in Consuelo
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in Consuelo.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names Consuelo expects.
</Note>
...
3. Wait for Consuelo to analyze your file

## Step 4: Map Your Columns

Consuelo automatically tries to match your columns to fields. Review and adjust:
...
3. Consuelo will link each Person to the matching Company
...
1. Consuelo shows your values alongside existing options
2. Match each value in your file to a Consuelo option
...
Before completing the import, Consuelo validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in Consuelo or file | Remove duplicate or update existing record |
...
Your contacts are now in Consuelo! Next steps:
...
2. Consuelo matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in Consuelo. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for Consuelo's internal ID.
  </Accordion>
