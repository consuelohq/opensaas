...
description: Step-by-step guide to migrate your data from any CRM to Consuelo.
---

## Overview

This guide walks you through migrating your data from any CRM to Consuelo. The process involves auditing your data, preparing your Consuelo workspace, exporting from your current system, and importing into Consuelo.
...
Create a mapping document between your current CRM and Consuelo:
...
## Step 3: Set Up Your Consuelo Workspace

Before importing data, prepare your Consuelo workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in Consuelo before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for Consuelo.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in Consuelo already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches Consuelo's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in Consuelo
...
- Consuelo will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in Consuelo.
  </Accordion>
...
Contact [contact@consuelohq.com](mailto:contact@consuelohq.com) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
