...
Consuelo를 처음 설치할 때, 기본 데이터베이스 비밀번호를 변경하고 싶을 수 있습니다.
...
Consuelo 설치 중에 적절한 스키마, 확장, 사용자를 사용하여 postgres 데이터베이스를 프로비저닝해야 합니다.
...
**작동하지 않는 경우:**
어떤 프로세스가 대부분의 머신 RAM을 소모하고 있는지 조사하십시오. Consuelo에서는 일부 VScode 확장이 많은 RAM을 소모하고 있음을 발견하여 임시로 비활성화했습니다.
...
* Consuelo 앱을 열고 `/settings`로 이동하여 화면 왼쪽 하단에 있는 **고급** 토글을 활성화하십시오.
* 새 웹훅 생성.
* **고유한 웹훅 URL**을 Consuelo의 **엔드포인트 URL** 필드에 붙여넣습니다. **필터**를 `회사`와 `생성됨`으로 설정하십시오.
...
Consuelo를 리버스 프록시 뒤에서 실행 중이며 연결 문제가 있는 경우:
...
2. **리버스 프록시 설정 확인:**
   ...
   * 리버스 프록시가 Consuelo 서버로 요청을 정확히 전달하는지 확인하십시오.
   * `X-Forwarded-For`, `X-Forwarded-Proto`와 같은 헤더가 적절히 설정되어 있는지 확인하십시오.

3. **서비스 재시작:**

   변경 후, 리버스 프록시와 Consuelo 컨테이너를 모두 다시 시작하십시오.
...
* 커뮤니티 지원:
  ...
  [Consuelo 커뮤니티](https://github.com/twentyhq/consuelo/issues)나 [지원 채널](https://discord.gg/cx5n4Jzs57)에 문의하십시오.
