...
<Warning>
  **처음 설치하시나요?** [Docker Compose 설치 가이드](/l/ko/developers/self-host/capabilities/docker-compose)를 따라 Consuelo를 실행하고, 이곳으로 돌아와 구성을 진행하세요.
</Warning>

Consuelo는 다른 배포 요구에 맞추기 위해 **두 가지 구성 모드**를 제공합니다:
...
1. Consuelo 인스턴스에 접근하십시오 (보통 `http://localhost:3000`)
...
<Warning>
  ...
  [완전한 기술적 참조 →](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-server/src/engine/core-modules/twenty-config/config-variables.ts)
</Warning>
...
기본적으로 Consuelo는 **단일 워크스페이스 모드**로 실행됩니다 — 조직에 하나의 CRM 인스턴스가 필요한 대부분의 자가 호스팅 배포에 적합합니다.
...
여러 독립적인 팀이 동일한 Consuelo 인스턴스에서 자체 워크스페이스가 필요한 SaaS형 배포에서 멀티 워크스페이스 모드를 활성화하세요.
...
**필요한 범위** (자동 구성됨):
[관련 소스 코드 보기](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  사용자는 캘린더 및 메시징 API를 사용하려면 [Microsoft 365 라이선스](https://admin.microsoft.com/Adminportal/Home)를 보유해야 합니다. 없이는 Consuelo에서 계정을 동기화할 수 없습니다.
</Warning>
...
[관련 소스 코드 보기](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
Consuelo는 워크플로 및 사용자 지정 로직을 위해 로직 함수를 지원합니다. `SERVERLESS_TYPE` 환경 변수를 통해 실행 환경을 구성합니다.
...
<Note>
  `SERVERLESS_TYPE=DISABLED`를 사용하는 경우 로직 함수를 실행하려는 모든 시도는 오류를 반환합니다. 로직 함수 기능 없이 Consuelo를 실행하려는 경우 유용합니다.
</Note>
