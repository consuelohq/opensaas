...
当您导入的文件包含与现有记录匹配的**唯一标识符**时，Consuelo 会更新该记录，而不是创建重复项。
...
2. 在您要更改的列中**更新这些值**
3. **移除不需要更新的列**（可选，但更简洁）
4. **不要更改唯一标识符的值** — 否则 Consuelo 将创建新记录
...
<Warning>
  **不要更改唯一标识符的值。**
  ...
  如果您将 `john@acme.com` 更改为 `john.smith@acme.com`，Consuelo 将创建新记录，而不是更新现有记录。
</Warning>
...
Consuelo 根据唯一标识符匹配记录，并使用新值进行更新。
...
| **大小写不匹配**  | 文件中为 `John@acme.com`，但 Consuelo 中为 `john@acme.com` | 创建新记录      | 从 Consuelo 导出以获取精确值            |
...
<AccordionGroup>
  ...
  <Accordion title="我如何知道哪些记录会被更新、哪些会被创建？">
    在导入审核步骤中，Consuelo 会根据唯一标识符的匹配情况，显示将被更新与被创建的记录数量。
  </Accordion>
