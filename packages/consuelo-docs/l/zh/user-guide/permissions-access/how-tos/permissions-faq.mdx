...
<AccordionGroup>
  <Accordion title="Consuelo 默认包含哪些角色？">
    Consuelo 默认提供 **管理员** 和 **成员** 角色。 您可以根据团队需求创建额外的自定义角色（例如：销售代表、经理、只读用户）。
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="什么是即时（JIT）预配？">
    启用 JIT 预配后，用户首次通过 SSO 登录时，会在 Consuelo 中自动创建其账户。 系统会自动为其分配默认角色。
  </Accordion>
