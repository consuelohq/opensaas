...
1. 在 Consuelo 运行的主机上打开终端，关闭 Consuelo：`docker compose down`

2. 通过更改 docker-compose 附近 .env 文件中的 `TAG` 值来升级版本。 （我们建议使用 `major.minor` 版本，例如 `v0.53`）

3. 用 `docker compose up -d` 重新启动 Consuelo
...
此问题仅适用于此 Consuelo 版本，未来的升级不应需要此操作。
...
它应始终在您当前 Consuelo 实例 `major.minor` 版本的范围内，您可以在管理面板中查看您的实例版本（在 `/settings/admin-panel`，如果您的用户在数据库中设置了 `canAccessFullAdminPanel` 属性为真则可访问）或通过在 `twenty-server` 容器中运行 `echo $APP_VERSION`。

要修复不同步的工作区版本，你将需要从相应的 Consuelo 版本按照相关的升级指南顺序升级，等等，直到到达所需的版本。
...
您将需要在数据库中手动更新工作区版本到 `0.51.0`，并使用 Consuelo 版本 `0.52.11` 进行升级，按照其上方的升级指南。

### v0.50 to v0.51

升级您的 Consuelo 实例以使用 v0.51 映像
...
升级您的 Consuelo 实例以使用 v0.50.0 映像
...
请使用 [v0.50.0 docker-compose.yml](https://github.com/twentyhq/consuelo/blob/v0.50.0/packages/twenty-docker/docker-compose.yml) 更新本地主机上的 `docker-compose.yml`

### v0.43.0 to v0.44.0

升级您的 Consuelo 实例以使用 v0.44.0 映像
...
升级您的 Consuelo 实例以使用 v0.43.0 映像
...
2. 将您的 docker-compose.yml 升级为与 [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/docker-compose.yml) 文件中的 postgres:16 映像相同。
...
升级您的 Consuelo 实例以使用 v0.42.0 映像
...
升级您的 Consuelo 实例以使用 v0.41.0 映像
...
升级您的 Consuelo 实例以使用 v0.40.0 映像
...
升级您的 Consuelo 实例以使用 v0.35.0 映像
...
升级您的 Consuelo 实例以使用 v0.34.0 映像
...
升级您的 Consuelo 实例以使用 v0.33.0 映像
...
如果要继续使用 twenty-postgres 映像，只需在 docker-compose.yml 中将 `Consuelo/twenty-postgres:${TAG}` 替换为 `Consuelo/twenty-postgres` 即可。

### v0.31.0 to v0.32.0

升级您的 Consuelo 实例以使用 v0.32.0 映像
...
升级您的 Consuelo 实例以使用 v0.31.0 映像
...
升级您的 Consuelo 实例以使用 v0.30.0 映像

**重大更改**：
为了提高性能，Consuelo 现在要求配置 Redis 缓存。 我们更新了我们的 [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/docker-compose.yml) 来反映这一点。
...
升级您的 Consuelo 实例以使用 v0.24.0 映像
...
升级您的 Consuelo 实例以使用 v0.23.0 映像
...
升级您的 Consuelo 实例以使用 v0.22.0 映像
