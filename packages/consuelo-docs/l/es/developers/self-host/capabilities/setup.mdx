...
<Warning>
  **¿Instalando por primera vez?** Siga la [guía de instalación de Docker Compose](/l/es/developers/self-host/capabilities/docker-compose) para ejecutar Consuelo, luego regrese aquí para la configuración.
</Warning>

Consuelo ofrece **dos modos de configuración** para adaptarse a diferentes necesidades de implementación:
...
1. Acceda a su instancia de Consuelo (normalmente `http://localhost:3000`)
...
<Warning>
  ...
  [Referencia técnica completa →](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-server/src/engine/core-modules/twenty-config/config-variables.ts)
</Warning>
...
De forma predeterminada, Consuelo se ejecuta en **modo de un solo espacio de trabajo** — ideal para la mayoría de las implementaciones autoalojadas en las que necesitas una instancia de CRM para tu organización.
...
* Un espacio de trabajo por instancia de Consuelo
...
Habilita el modo de múltiples espacios de trabajo para implementaciones tipo SaaS en las que varios equipos independientes necesitan sus propios espacios de trabajo en la misma instancia de Consuelo.
...
[Ver código fuente relevante](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Los usuarios deben tener una [licencia de Microsoft 365](https://admin.microsoft.com/Adminportal/Home) para poder usar la API de Calendar y Messaging. No podrán sincronizar su cuenta en Consuelo sin una.
</Warning>
...
[Ver código fuente relevante](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
Consuelo admite funciones de lógica para flujos de trabajo y lógica personalizada. El entorno de ejecución se configura mediante la variable de entorno `SERVERLESS_TYPE`.
...
<Note>
  Al usar `SERVERLESS_TYPE=DISABLED`, cualquier intento de ejecutar una función de lógica devolverá un error. Esto es útil si desea ejecutar Consuelo sin capacidades de funciones de lógica.
</Note>
