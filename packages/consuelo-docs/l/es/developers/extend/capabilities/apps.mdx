---
title: Aplicaciones de Consuelo
description: Crea y gestiona personalizaciones de Consuelo como código.
---
...
Las aplicaciones te permiten crear y administrar personalizaciones de Consuelo **como código**. En lugar de configurar todo a través de la interfaz de usuario, defines tu modelo de datos y funciones de lógica en código, lo que hace más rápido crear, mantener y desplegar en múltiples espacios de trabajo.
...
* Node.js 24+ y Yarn 4
* Un espacio de trabajo de Consuelo y una clave de API (créala en https://app.consuelo.com/settings/api-webhooks)
...
* `yarn app:generate` creará una carpeta `generated/` (cliente tipado de Consuelo + tipos del espacio de trabajo).
...
<Note>
  **Los campos base se crean automáticamente.** Cuando defines un objeto personalizado, Consuelo añade automáticamente campos estándar como `name`, `createdAt`, `updatedAt`, `createdBy`, `position` y `deletedAt`. No necesitas definir estos en tu matriz `fields` — solo agrega tus campos personalizados.
</Note>
...
* Consulta un ejemplo funcional en la aplicación Hello World: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
Cuando tu función se ejecuta en Consuelo, la plataforma inyecta credenciales como variables de entorno antes de que tu código se ejecute:

* `TWENTY_API_URL`: URL base de la API de Consuelo a la que apunta tu aplicación.
...
Explora un ejemplo mínimo de extremo a extremo que demuestra objetos, funciones y múltiples desencadenadores [aquí](https://github.com/consuelohq/opensaas/tree/main/packages/twenty-apps/hello-world):
...
* No se puede conectar al servidor: verifica la URL de la API y que el servidor de Consuelo sea accesible.
