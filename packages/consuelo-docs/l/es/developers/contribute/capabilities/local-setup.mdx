...
description: La guía para los colaboradores (o desarrolladores curiosos) que quieren ejecutar Consuelo localmente.
---
...
<Tabs>
  <Tab title="Linux y MacOS">
    Antes de que puedas instalar y usar Consuelo, asegúrate de instalar lo siguiente en tu computadora:
...
Consuelo requiere un caché de redis para proporcionar el mejor rendimiento
...
El frontend de Consuelo estará ejecutándose en [http://localhost:3001](http://localhost:3001).
Puede iniciar sesión usando la cuenta demo por defecto: `tim@apple.dev` (contraseña: `tim@apple.dev`)

**Backend**

* El servidor de Consuelo estará operativo en [http://localhost:3000](http://localhost:3000)
