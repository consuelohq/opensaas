...
### Relaciones comunes en Consuelo
...
El padre (lado "uno") debe existir en Consuelo antes de poder hacer referencia a él.
...
1. Exporte los registros padre desde Consuelo
...
<Warning>
  **¡El formato del dominio importa!**
  ...
  Use `https://domain.com` (no solo `domain.com`). Esto coincide con cómo Consuelo almacena los dominios de las Empresas y evita errores de coincidencia.
</Warning>
...
Consuelo vinculará automáticamente cada registro hijo con el padre correspondiente.
...
| **Registros inexistentes**                | El registro padre no existe en Consuelo                | Importe primero los registros padre o verifique si hay errores tipográficos |
| **Diferencia de mayúsculas y minúsculas** | `Acme.com` en el archivo pero `acme.com` en Consuelo   | Asegure una coincidencia exacta de mayúsculas y minúsculas                  |
...
<AccordionGroup>
  <Accordion title="¿Qué pasa si mi registro padre no tiene dominio ni correo electrónico?">
    ...
    1. Use el `id` de Consuelo (exporte los registros padre para obtener sus IDs)
