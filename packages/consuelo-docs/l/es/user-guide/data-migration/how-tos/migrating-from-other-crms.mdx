...
description: Guía paso a paso para migrar tus datos desde cualquier CRM a Consuelo.
---

## Resumen

Esta guía te lleva paso a paso para migrar tus datos desde cualquier CRM a Consuelo. El proceso implica auditar tus datos, preparar tu espacio de trabajo de Consuelo, exportar desde tu sistema actual e importar en Consuelo.
...
Crea un documento de mapeo entre tu CRM actual y Consuelo:
...
* El campo de destino en Consuelo
...
## Paso 3: Configura tu espacio de trabajo en Consuelo

Antes de importar datos, prepara tu espacio de trabajo en Consuelo:
...
<Warning>
  **Invita a los usuarios ANTES de importar datos.**
  ...
  Si tus datos incluyen referencias a usuarios (propietario de la cuenta, asignado, etc.), esos usuarios deben existir en Consuelo antes de la importación. De lo contrario, esas relaciones no se podrán asignar.
</Warning>
...
Abre cada CSV exportado en una aplicación de hojas de cálculo y prepáralo para Consuelo.

### Elimina duplicados

1. Ordena por el campo único (correo electrónico para Personas, dominio para Empresas)
2. Elimina o combina filas duplicadas
3. Verifica que no existan duplicados ya en Consuelo
...
<Warning>
  **El formato del dominio es fundamental.**
  ...
  Usa `https://domain.com` (no `domain.com` ni `www.domain.com`). Esto coincide con el formato de Consuelo y evita duplicados cuando conectas la sincronización de correo/calendario.
</Warning>
...
2. Usa las mismas direcciones de correo electrónico que los usuarios utilizaron para unirse a tu espacio de trabajo en Consuelo
...
1. Navega hasta el objeto en Consuelo
...
* Consuelo comenzará a sincronizar los correos electrónicos con los registros de contactos
...
<AccordionGroup>
  <Accordion title="¿Puedo migrar los flujos de trabajo automáticamente?">
    Actualmente no. Los flujos de trabajo deben recrearse manualmente en Consuelo.
  </Accordion>
...
Contacta con [contact@consuelo.com](mailto:contact@consuelo.com) o explora nuestros [Servicios de implementación](/l/es/user-guide/getting-started/capabilities/implementation-services).
