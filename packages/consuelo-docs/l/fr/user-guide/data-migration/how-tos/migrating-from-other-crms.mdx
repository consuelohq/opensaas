...
description: Guide pas à pas pour migrer vos données de n'importe quel CRM vers Consuelo.
---

## Vue d'ensemble

Ce guide vous accompagne pour migrer vos données depuis n'importe quel CRM vers Consuelo. Le processus consiste à auditer vos données, à préparer votre espace de travail sur Consuelo, à exporter depuis votre système actuel et à importer dans Consuelo.
...
Créez un document de correspondance entre votre CRM actuel et Consuelo :
...
## Étape 3 : Configurez votre espace de travail Consuelo

Avant d'importer des données, préparez votre espace de travail Consuelo :
...
<Warning>
  **Invitez les utilisateurs AVANT d'importer des données.**
  ...
  Si vos données incluent des références à des utilisateurs (propriétaire du compte, assigné, etc.), ces utilisateurs doivent exister dans Consuelo avant l'importation. Sinon, ces relations ne pourront pas être mises en correspondance.
</Warning>
...
Ouvrez chaque CSV exporté dans une application de tableur et préparez-le pour Consuelo.
...
3. Vérifiez qu'il n'existe pas déjà de doublons dans Consuelo
...
<Warning>
  **Le format du domaine est critique.**
  ...
  Utilisez `https://domain.com` (pas `domain.com` ni `www.domain.com`). Cela correspond au format de Consuelo et évite les doublons lorsque vous connectez la synchronisation e-mail/calendrier.
</Warning>
...
2. Utilisez les mêmes adresses e-mail que celles utilisées par les utilisateurs pour rejoindre votre espace de travail Consuelo
...
## Étape 6 : Importez dans Consuelo
...
1. Accédez à l'objet dans Consuelo
...
* Consuelo commencera à synchroniser les e-mails avec les enregistrements de contacts
...
<AccordionGroup>
  <Accordion title="Puis-je migrer les flux de travail automatiquement ?">
    Pas pour le moment. Les flux de travail doivent être recréés manuellement dans Consuelo.
  </Accordion>
...
Contactez [contact@consuelo.com](mailto:contact@consuelo.com) ou découvrez nos [services de mise en œuvre](/l/fr/user-guide/getting-started/capabilities/implementation-services).
