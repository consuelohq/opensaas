...
| **Intégrations personnalisées** | Votre application → automatisation dans Consuelo                 |
...
3. Vous recevrez une URL de webhook unique, par exemple :
   ```
   https://api.consuelo.com/webhooks/workflow/abc123...
   ```
...
```bash
curl -X POST https://api.consuelo.com/webhooks/workflow/abc123... \
