...
1. Dans un terminal, sur l'hôte où Consuelo est en cours d'exécution, éteignez Consuelo: `docker compose down`
...
3. Remettez Consuelo en ligne avec `docker compose up -d`
...
Mettez à niveau votre instance Consuelo pour utiliser l'image v0.55
...
Ce problème est spécifique à cette version de Consuelo et ne devrait pas être nécessaire pour les futures mises à niveau.
...
Il doit toujours être dans la plage de la version `major.minor` de votre instance Consuelo actuelle, vous pouvez consulter la version de votre instance dans le panneau d'administration (à l'adresse `/settings/admin-panel`, accessible si votre utilisateur a la propriété `canAccessFullAdminPanel` définie comme vraie dans la base de données) ou en exécutant `echo $APP_VERSION` dans votre conteneur `twenty-server`.

Pour corriger une version d'espace de travail désynchronisée, vous devrez effectuer une mise à niveau à partir de la version correspondante de Consuelo en suivant le guide de mise à niveau correspondant séquentiellement, et ainsi de suite jusqu'à atteindre la version souhaitée.
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.52
...
Vous devrez mettre à jour manuellement la version de votre espace de travail à `0.51.0` dans la base de données et mettre à niveau en utilisant la version consuelo `0.52.11` en suivant son guide de mise à niveau juste au-dessus.

### v0.50 à v0.51

Mettez à jour votre instance Consuelo pour utiliser l'image v0.51
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.50.0
...
Veuillez mettre à jour votre `docker-compose.yml` local avec [docker-compose.yml v0.50.0](https://github.com/consuelohq/opensaas/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)

### v0.43.0 à v0.44.0

Mettez à jour votre instance Consuelo pour utiliser l'image v0.44.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.43.0
...
2. Mettez à jour votre docker-compose.yml pour utiliser l'image postgres:16 comme dans le fichier [docker-compose.yml](https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/docker-compose.yml).
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.42.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.41.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.40.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.35.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.34.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.33.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.32.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.31.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.30.0

**Changement significatif**:
Pour améliorer les performances, Consuelo nécessite désormais la configuration de cache redis. Nous avons mis à jour notre [docker-compose.yml](https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/docker-compose.yml) pour refléter cela.
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.24.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.23.0
...
Mettez à jour votre instance Consuelo pour utiliser l'image v0.22.0
