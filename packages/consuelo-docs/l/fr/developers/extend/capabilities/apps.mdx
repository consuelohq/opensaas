---
title: Applications Consuelo
description: Créez et gérez les personnalisations Consuelo sous forme de code.
---
...
Les applications vous permettent de créer et de gérer des personnalisations Consuelo **sous forme de code**. Au lieu de tout configurer via l’interface utilisateur, vous définissez votre modèle de données et vos fonctions logiques dans le code — ce qui accélère la création, la maintenance et le déploiement sur plusieurs espaces de travail.
...
* Un espace de travail Consuelo et une clé API (créez-en une sur https://app.consuelo.com/settings/api-webhooks)
...
* `yarn app:generate` créera un dossier `generated/` (client Consuelo typé + types de l’espace de travail).
...
<Note>
  **Les champs de base sont créés automatiquement.** Lorsque vous définissez un objet personnalisé, Consuelo ajoute automatiquement des champs standard tels que `name`, `createdAt`, `updatedAt`, `createdBy`, `position` et `deletedAt`. Vous n'avez pas besoin de les définir dans votre tableau `fields` — ajoutez uniquement vos champs personnalisés.
</Note>
...
* Voir un exemple fonctionnel dans l’application Hello World : [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
Lorsque votre fonction s’exécute sur Consuelo, la plateforme injecte des identifiants sous forme de variables d’environnement avant l’exécution de votre code :

* `TWENTY_API_URL` : URL de base de l’API Consuelo ciblée par votre application.
...
Découvrez un exemple minimal de bout en bout qui démontre des objets, des fonctions et plusieurs déclencheurs [ici](https://github.com/consuelohq/opensaas/tree/main/packages/twenty-apps/hello-world) :
...
* Impossible de se connecter au serveur : vérifiez l’URL de l’API et que le serveur Consuelo est accessible.
