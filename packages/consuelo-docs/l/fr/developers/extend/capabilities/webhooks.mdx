...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans Consuelo — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
Consuelo envoie des webhooks pour ces types d'événements :
...
Consuelo signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans Consuelo depuis des systèmes externes                         |
