...
<AccordionGroup>
  <Accordion title="Quali ruoli sono inclusi in Consuelo per impostazione predefinita?">
    Consuelo include per impostazione predefinita i ruoli **Admin** e **Member**. Puoi creare ulteriori ruoli personalizzati in base alle esigenze del tuo team (ad es., Addetto alle vendite, Manager, Utente in sola lettura).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Quali permessi relativi alle azioni sono disponibili?">
    ...
    * **Invia email**: Possibilità di inviare email da Consuelo
...
<AccordionGroup>
  ...
  <Accordion title="Che cos'è il provisioning Just-in-Time (JIT)?">
    Con il provisioning JIT, gli account utente vengono creati automaticamente in Consuelo quando qualcuno accede tramite SSO per la prima volta. Viene assegnato loro automaticamente il ruolo predefinito.
  </Accordion>
