...
description: Guida passo-passo per migrare i tuoi dati da qualsiasi CRM a Consuelo.
---

## Panoramica

Questa guida ti accompagna nella migrazione dei tuoi dati da qualsiasi CRM a Consuelo. Il processo prevede la revisione dei tuoi dati, la preparazione del tuo spazio di lavoro Consuelo, l'esportazione dal sistema attuale e l'importazione in Consuelo.
...
Crea un documento di mappatura tra il tuo CRM attuale e Consuelo:
...
* Il campo di destinazione di Consuelo
...
## Passaggio 3: Configura il tuo spazio di lavoro Consuelo

Prima di importare i dati, prepara il tuo spazio di lavoro Consuelo:
...
<Warning>
  **Invita gli utenti PRIMA di importare i dati.**
  ...
  Se i tuoi dati includono riferimenti a utenti (Proprietario dell'account, Assegnatario, ecc.), tali utenti devono esistere in Consuelo prima dell'importazione. In caso contrario, tali relazioni non possono essere mappate.
</Warning>
...
Apri ogni file CSV esportato in un'applicazione di fogli di calcolo e preparalo per Consuelo.

### Rimuovi i duplicati

1. Ordina per il campo univoco (email per Persone, dominio per Aziende)
2. Rimuovi o unisci le righe duplicate
3. Verifica che non esistano già duplicati in Consuelo
...
<Warning>
  **Il formato del dominio è fondamentale.**
  ...
  Usa `https://domain.com` (non `domain.com` o `www.domain.com`). Questo corrisponde al formato di Consuelo ed evita duplicati quando colleghi la sincronizzazione di email e calendario.
</Warning>
...
2. Usa gli stessi indirizzi email che gli utenti hanno utilizzato per unirsi al tuo spazio di lavoro Consuelo
...
## Passaggio 6: Importa in Consuelo
...
* Consuelo inizierà a sincronizzare le email con i record dei contatti
...
<AccordionGroup>
  <Accordion title="Posso migrare i flussi di lavoro automaticamente?">
    Al momento no. I flussi di lavoro devono essere ricreati manualmente in Consuelo.
  </Accordion>
...
Contatta [contact@consuelo.com](mailto:contact@consuelo.com) oppure scopri i nostri [Servizi di implementazione](/l/it/user-guide/getting-started/capabilities/implementation-services).
