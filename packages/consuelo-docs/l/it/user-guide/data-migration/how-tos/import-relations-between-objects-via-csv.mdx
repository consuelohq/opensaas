...
Il genitore (lato "uno") deve esistere in Consuelo prima di potervi fare riferimento.
...
1. Esporta i record genitore da Consuelo
...
<Warning>
  **Il formato del dominio è importante!**
  ...
  Usa `https://domain.com` (non solo `domain.com`). Questo corrisponde a come Consuelo archivia i domini delle Aziende e previene errori di corrispondenza.
</Warning>
...
Consuelo collegherà automaticamente ogni record figlio al genitore corrispondente.
...
| **Record inesistenti**                    | Il record genitore non esiste in Consuelo           | Importa prima i record genitore oppure verifica la presenza di errori di battitura |
| **Discrepanza tra maiuscole e minuscole** | `Acme.com` nel file ma `acme.com` in Consuelo       | Assicurati che le maiuscole/minuscole corrispondano esattamente                    |
...
<AccordionGroup>
  <Accordion title="Cosa succede se il mio record genitore non ha un dominio o un'email?">
    ...
    1. Usa l'`id` di Consuelo (esporta i record genitore per ottenere i loro ID)
