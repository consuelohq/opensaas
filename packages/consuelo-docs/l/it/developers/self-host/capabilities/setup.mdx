...
<Warning>
  **Prima volta che installi?** Segui la [guida all'installazione di Docker Compose](/l/it/developers/self-host/capabilities/docker-compose) per far funzionare Consuelo, quindi torna qui per la configurazione.
</Warning>

Consuelo offre **due modalità di configurazione** per soddisfare diverse esigenze di distribuzione:
...
1. Accedi alla tua istanza Consuelo (solitamente `http://localhost:3000`)
...
<Warning>
  ...
  [Riferimento tecnico completo →](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-server/src/engine/core-modules/twenty-config/config-variables.ts)
</Warning>
...
Per impostazione predefinita, Consuelo viene eseguito in **modalità a workspace singolo** — ideale per la maggior parte delle distribuzioni self-hosted in cui è necessaria un'istanza di CRM per la tua organizzazione.
...
* Un workspace per istanza di Consuelo
...
Abilita la modalità multi-workspace per distribuzioni in stile SaaS in cui più team indipendenti necessitano dei propri workspace sulla stessa istanza di Consuelo.
...
**Scope richiesti** (configurati automaticamente): [Vedi il codice sorgente pertinente](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Gli utenti devono avere una [Licenza Microsoft 365](https://admin.microsoft.com/Adminportal/Home) per poter utilizzare l'API Calendario e Messaggi. Non potranno sincronizzare il loro account su Consuelo senza una.
</Warning>
...
[Vedi il codice sorgente pertinente](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
Consuelo supporta le funzioni logiche per i workflow e la logica personalizzata. L'ambiente di esecuzione è configurato tramite la variabile di ambiente `SERVERLESS_TYPE`.
...
<Note>
  Quando si utilizza `SERVERLESS_TYPE=DISABLED`, qualsiasi tentativo di eseguire una funzione logica restituirà un errore. Ciò è utile se si desidera eseguire Consuelo senza il supporto per le funzioni logiche.
</Note>
