...
Se hai un'istanza di Consuelo esistente con dati di produzione, **NON** seguire questi passaggi poiché elimineranno permanentemente il tuo database!

Durante l'installazione di Consuelo per la prima volta, potresti voler cambiare la password del database predefinita.
...
Durante l'installazione di Consuelo, devi fornire il tuo database postgres con gli schemi, le estensioni e gli utenti corretti.
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. In Consuelo, abbiamo notato che alcune estensioni di VScode stavano occupando molta RAM quindi le disabilitiamo temporaneamente.
...
* Apri la tua app Consuelo, naviga su `/settings` e abilita il toggle **Avanzate** in basso a sinistra dello schermo.
* Crea un nuovo webhook.
* Incolla **Your Unique Webhook URL** nel campo **Endpoint Url** in Consuelo. Imposta i **Filtri** su `Companies` e `Created`.
...
Se stai eseguendo Consuelo dietro un reverse proxy e stai riscontrando problemi di connessione:
...
2. **Controlla le impostazioni del reverse proxy:**
   ...
   * Conferma che il tuo reverse proxy sta inoltrando correttamente le richieste al server Consuelo.
   * Assicurati che le intestazioni come `X-Forwarded-For` e `X-Forwarded-Proto` siano impostate correttamente.

3. **Riavvia i servizi:**

   Dopo aver apportato modifiche, riavvia sia il reverse proxy che i container Consuelo.
...
* Supporto comunitario:
  ...
  Contatta la [comunità Consuelo](https://github.com/consuelohq/opensaas/issues) o [canali di supporto](https://discord.gg/cx5n4Jzs57) per assistenza.
