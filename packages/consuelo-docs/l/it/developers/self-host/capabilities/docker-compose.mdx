...
Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione Consuelo usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Installa l'ultima versione stabile di Consuelo con un unico comando:
...
```bash
VERSION=vx.y.z BRANCH=nome-ramo bash <(curl -sL https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/scripts/install.sh)
```
...
Se ospiti Consuelo sul tuo computer, apri il browser e naviga a [http://localhost:3000](http://localhost:3000).
...
### Esponi Consuelo per Accesso Esterno

Per impostazione predefinita, Consuelo gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare Consuelo dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
