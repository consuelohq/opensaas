...
description: La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire Consuelo localmente.
---
...
<Tabs>
  <Tab title="Linux e MacOS">
    Prima di poter installare e usare Consuelo, assicurati di installare quanto segue sul tuo computer:
...
Consuelo richiede una cache Redis per offrire le migliori prestazioni
...
## Passo 8: Utilizza Consuelo

**Frontend**

Il frontend di Consuelo sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

* Il server di Consuelo sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
