---
title: App di Consuelo
description: Crea e gestisci le personalizzazioni di Consuelo come codice.
---
...
Le app ti consentono di creare e gestire le personalizzazioni di Consuelo **come codice**. Invece di configurare tutto tramite l'interfaccia utente, definisci in codice il modello dati e le funzioni logiche — rendendo più veloce creare, mantenere e distribuire su più spazi di lavoro.
...
* Node.js 24+ e Yarn 4
* Uno spazio di lavoro Consuelo e una chiave API (creane una su https://app.consuelo.com/settings/api-webhooks)
...
* `yarn twenty app:generate` will create a `generated/` folder (typed Consuelo client + workspace types).
...
<Note>
  **I campi base vengono creati automaticamente.** Quando definisci un oggetto personalizzato, Consuelo aggiunge automaticamente i campi standard come `name`, `createdAt`, `updatedAt`, `createdBy`, `position` e `deletedAt`. Non è necessario definirli nel tuo array `fields` — aggiungi solo i tuoi campi personalizzati.
</Note>
...
* Vedi un esempio funzionante nell'app Hello World: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
I componenti front-end ti consentono di creare componenti React personalizzati che vengono renderizzati all'interno dell'interfaccia di Consuelo. Usa `defineFrontComponent()` per definire componenti con convalida integrata:
...
* I componenti front-end sono componenti React che eseguono il rendering in contesti isolati all'interno di Consuelo.
...
Quando la tua funzione viene eseguita su Consuelo, la piattaforma inietta le credenziali come variabili d'ambiente prima dell'esecuzione del tuo codice:

* `TWENTY_API_URL`: URL di base dell'API Consuelo a cui punta la tua app.
...
Esplora un esempio minimale end-to-end che dimostra oggetti, funzioni logiche, componenti front-end e trigger multipli [qui](https://github.com/consuelohq/opensaas/tree/main/packages/twenty-apps/hello-world):
...
* Impossibile connettersi al server: verifica l'URL dell'API e che il server Consuelo sia raggiungibile.
