...
Os webhooks enviam dados para seus sistemas em tempo real quando eventos ocorrem no Consuelo — sem necessidade de polling. Use-os para manter sistemas externos em sincronia, acionar automações ou enviar alertas.
...
O Consuelo envia webhooks para estes tipos de eventos:
...
O Consuelo assina cada solicitação de webhook por segurança. Valide as assinaturas para garantir que as solicitações sejam autênticas.

### Headers

| Cabeçalho                    | Descrição                |
| ---------------------------- | ------------------------ |
| `X-Consuelo-Webhook-Signature` | Assinatura HMAC SHA256   |
| `X-Consuelo-Webhook-Timestamp` | Timestamp da solicitação |

### Etapas de validação

1. Obtenha o timestamp de `X-Consuelo-Webhook-Timestamp`
2. Crie a string: `{timestamp}:{JSON payload}`
3. Calcule o HMAC SHA256 usando o segredo do seu webhook
4. Compare com `X-Consuelo-Webhook-Signature`
...
| **Gatilho de webhook de fluxo de trabalho** | ENTRADA | Receber dados no Consuelo a partir de sistemas externos                            |
