...
1. No terminal, no host onde o Consuelo está em execução, desligue o Consuelo: `docker compose down`
...
3. Traga o Consuelo de volta ao ar com `docker compose up -d`
...
Atualize sua instância do Consuelo para usar a imagem v0.55
...
Este problema é específico para esta versão do Consuelo e não deverá ser necessário em futuras atualizações.
...
Deve estar sempre na faixa da versão `major.minor` atual da instância do Consuelo, você pode ver a versão de sua instância no painel de administração (em `/settings/admin-panel`, acessível se seu usuário tiver a propriedade `canAccessFullAdminPanel` definida como verdadeira no banco de dados) ou executando `echo $APP_VERSION` em seu contêiner `twenty-server`.

Para corrigir uma versão de workspace dessincronizada, você terá que atualizar da versão correspondente do consuelo seguindo o guia de atualização relacionado sequencialmente e assim por diante até alcançar a versão desejada.
...
Atualize sua instância do Consuelo para usar a imagem v0.52
...
Você terá que atualizar manualmente a versão do espaço de trabalho para `0.51.0` no banco de dados e atualizar usando a versão consuelo `0.52.11` seguindo o guia de atualização logo acima.

### v0.50 para v0.51

Atualize sua instância do Consuelo para usar a imagem v0.51
...
Atualize sua instância do Consuelo para usar a imagem v0.50.0
...
Por favor, atualize seu `docker-compose.yml` local com [v0.50.0 docker-compose.yml](https://github.com/twentyhq/consuelo/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)

### v0.43.0 para v0.44.0

Atualize sua instância do Consuelo para usar a imagem v0.44.0
...
Atualize sua instância do Consuelo para usar a imagem v0.43.0
...
2. Atualize seu docker-compose.yml para usar a imagem postgres:16 como no arquivo [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/docker-compose.yml).
...
Atualize sua instância do Consuelo para usar a imagem v0.42.0
...
Atualize sua instância do Consuelo para usar a imagem v0.41.0
...
Atualize sua instância do Consuelo para usar a imagem v0.40.0
...
Atualize sua instância do Consuelo para usar a imagem v0.35.0
...
Atualize sua instância do Consuelo para usar a imagem v0.34.0
...
Atualize sua instância do Consuelo para usar a imagem v0.33.0
...
Se você quiser continuar usando a imagem twenty-postgres, basta substituir `Consuelo/twenty-postgres:${TAG}` por `Consuelo/twenty-postgres` em docker-compose.yml.

### v0.31.0 para v0.32.0

Atualize sua instância do Consuelo para usar a imagem v0.32.0
...
Atualize sua instância do Consuelo para usar a imagem v0.31.0
...
Atualize sua instância do Consuelo para usar a imagem v0.30.0

**Mudança radical**:
Para melhorar o desempenho, o Consuelo agora requer que o cache redis seja configurado. Atualizamos nosso [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/docker-compose.yml) para refletir isso.
...
Atualize sua instância do Consuelo para usar a imagem v0.24.0
...
Atualize sua instância do Consuelo para usar a imagem v0.23.0
...
Atualize sua instância do Consuelo para usar a imagem v0.22.0
