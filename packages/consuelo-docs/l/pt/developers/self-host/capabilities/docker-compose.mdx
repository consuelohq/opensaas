...
Este guia fornece instruções passo a passo para instalar e configurar o aplicativo Consuelo usando Docker Compose. O objetivo é tornar o processo simples e evitar armadilhas comuns que possam quebrar sua configuração.
...
Instale a versão estável mais recente do Consuelo com um único comando:
...
Se você hospedar o Consuelo no seu próprio computador, abra o navegador e acesse [http://localhost:3000](http://localhost:3000).
...
### Expor o Consuelo para Acesso Externo

Por padrão, o Consuelo é executado em `localhost` na porta `3000`. Para acessá-lo via um domínio externo ou endereço IP, você precisa configurar o `SERVER_URL` no seu arquivo `.env`.
...
Recomendamos fortemente configurar o Consuelo atrás de um proxy reverso com terminação SSL para segurança e funcionalidade ótimas.
