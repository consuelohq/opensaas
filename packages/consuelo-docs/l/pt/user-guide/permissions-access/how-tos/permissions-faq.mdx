...
<AccordionGroup>
  <Accordion title="Quais funções o Consuelo traz por padrão?">
    O Consuelo vem com as funções **Admin** e **Member** por padrão. Você pode criar funções personalizadas adicionais com base nas necessidades da sua equipe (por exemplo, Representante de vendas, Gerente, Usuário somente leitura).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Quais permissões de ação estão disponíveis?">
    ...
    * **Enviar e-mail**: Capacidade de enviar e-mails a partir do Consuelo
...
<AccordionGroup>
  ...
  <Accordion title="O que é o provisionamento Just-in-Time (JIT)?">
    Com o provisionamento JIT, as contas de usuário são criadas automaticamente no Consuelo quando alguém faz login via SSO pela primeira vez. A função padrão é atribuída automaticamente.
  </Accordion>
