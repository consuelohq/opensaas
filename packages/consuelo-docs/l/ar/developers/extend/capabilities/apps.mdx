---
title: تطبيقات Consuelo
description: أنشئ وأدِر تخصيصات Consuelo على هيئة كود.
---
...
تتيح لك التطبيقات إنشاء وإدارة تخصيصات Consuelo **ككود**. بدلًا من تكوين كل شيء عبر واجهة المستخدم، تُعرِّف نموذج بياناتك ووظائف المنطق في الكود — مما يجعل البناء والصيانة والنشر إلى مساحات عمل متعددة أسرع.
...
* Node.js 24+ وYarn 4
* مساحة عمل Consuelo ومفتاح واجهة برمجة التطبيقات (أنشئ واحدًا على https://app.consuelo.com/settings/api-webhooks)
...
<Note>
  **يتم إنشاء الحقول الأساسية تلقائيًا.** عند تعريف كائن مخصص، يضيف Consuelo تلقائيًا حقولًا قياسية مثل `name` و`createdAt` و`updatedAt` و`createdBy` و`position` و`deletedAt`. لا تحتاج إلى تعريف هذه في مصفوفة `fields` — أضف فقط حقولك المخصصة.
</Note>
...
* اطّلع على مثال عملي في تطبيق Hello World: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
تتيح لك المكوّنات الأمامية إنشاء مكوّنات React مخصّصة تُعرَض داخل واجهة مستخدم Consuelo. استخدم `defineFrontComponent()` لتعريف مكوّنات مع تحقّق مدمج:
...
* المكوّنات الأمامية هي مكوّنات React تُعرَض ضمن سياقات معزولة داخل Consuelo.
...
عندما تعمل وظيفتك على Consuelo، يقوم النظام الأساسي بحقن بيانات الاعتماد كمتغيرات بيئة قبل تنفيذ كودك:

* `TWENTY_API_URL`: عنوان URL الأساسي لواجهة Consuelo البرمجية التي يستهدفها تطبيقك.
...
استكشف مثالًا بسيطًا شاملًا من البداية إلى النهاية يوضح الكائنات والوظائف المنطقية والمكوّنات الأمامية ومشغّلات متعددة [هنا](https://github.com/consuelohq/opensaas/tree/main/packages/twenty-apps/hello-world):
...
* يتعذّر الاتصال بالخادم: تحقق من عنوان URL لواجهة البرمجة وأن خادم Consuelo قابل للوصول.
