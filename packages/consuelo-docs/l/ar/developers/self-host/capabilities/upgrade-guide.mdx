...
1. في الطرفية، على الجهاز الذي يعمل فيه Consuelo، قم بإيقاف Consuelo: `docker compose down`

2. قم بترقية الإصدار عن طريق تغيير قيمة `TAG` في ملف .env بجانب docker-compose. ( نوصي باستخدام إصدار `major.minor` مثل `v0.53` )

3. قم بإعادة تشغيل Consuelo باستخدام `docker compose up -d`
...
إذا واجهت أي مشاكل في وقت التشغيل بعد الترقية، قد تحتاج إلى مسح التخزين المؤقت لضمان تزامنه مع أحدث التغييرات. قم بتشغيل هذا الأمر في حاوية خادم consuelo الخاص بك:
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.55
...
هذه المشكلة خاصة بهذا الإصدار من Consuelo ولا يجب أن تكون ضرورية في الترقيات المستقبلية.
...
يجب أن تكون دائمًا في نطاق إصدار `major.minor` لحساب Consuelo الحالي الخاص بك ، ويمكنك مشاهدة نسخة حسابك في لوحة المدير (في `/settings/admin-panel`، يمكن الوصول إليها إذا كانت خاصية `canAccessFullAdminPanel` الخاصة بالمستخدم مصفوفة إلى true في قاعدة البيانات) أو عن طريق تشغيل `echo $APP_VERSION` في حاوية `twenty-server` الخاصة بك.

لإصلاح إصدار مساحة العمل غير المتزامن ، سيتعين عليك الترقية من الإصدار المعني لـ Consuelo باتباع دليل الترقية الخاص ذو الصلة تسلسليًا وهكذا حتى يصل إلى الإصدار المطلوب.
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.52
...
سيتعين عليك تحديث نسخة مساحة العمل يدويًا إلى `0.51.0` في قاعدة البيانات والترقية باستخدام إصدار consuelo عند `0.52.11` باتباع دليل الترقية الخاص به أعلاه.

### من v0.50 إلى v0.51

قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.51
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.50.0
...
يرجى تحديث `docker-compose.yml` المحلي الخاص بك بـ [docker-compose.yml v0.50.0](https://github.com/consuelohq/opensaas/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)

### من v0.43.0 إلى v0.44.0

قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.44.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.43.0
...
2. قم بترقية docker-compose.yml الخاص بك لاستخدام صورة postgres:16 كما هو في الملف [docker-compose.yml](https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/docker-compose.yml).
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.42.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.41.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.40.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.35.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.34.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.33.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.32.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.31.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.30.0

**تغيير كبير**:
لتحسين الأداء، يتطلب Consuelo الآن تكوين Redis للتخزين المؤقت. قمنا بتحديث [docker-compose.yml](https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/docker-compose.yml) لتعكس ذلك.
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.24.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.23.0
...
قم بترقية مثيل Consuelo الخاص بك لاستخدام صورة v0.22.0
