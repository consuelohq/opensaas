...
Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația Consuelo folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Instalați cea mai recentă versiune stabilă a Consuelo cu o singură comandă:

```bash
bash <(curl -sL https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/scripts/install.sh)
```

Pentru a instala o versiune sau ramură specifică:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/scripts/install.sh)
```
...
1. **Creați Fișierul .env**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/twentyhq/consuelo/refs/heads/main/packages/twenty-docker/.env.example
   ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/twentyhq/consuelo/refs/heads/main/packages/twenty-docker/docker-compose.yml
```
...
Dacă găzduiți Consuelo pe propriul computer, deschideți browserul și navigați la [http://localhost:3000](http://localhost:3000).
...
### Expuneți Consuelo pentru Acces Extern

Implicit, Consuelo rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți Consuelo pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea Consuelo din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
...
```bash
docker exec twenty-postgres pg_dump -U postgres consuelo > backup_$(date +%Y%m%d).sql
```
...
```bash
0 2 * * * docker exec twenty-postgres pg_dump -U postgres consuelo > /backups/twenty_$(date +\%Y\%m\%d).sql
```
...
```bash
docker exec -i twenty-postgres psql -U postgres consuelo < backup_20240115.sql
```
