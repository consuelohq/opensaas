...
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze Consuelo local.
---
...
<Tabs>
  <Tab title="Linux și MacOS">
    Înainte de a instala și utiliza Consuelo, asigurați-vă că instalați următoarele pe computerul dvs.:
...
Consuelo necesită un cache Redis pentru a oferi cea mai bună performanță
...
## Pasul 8: Utilizați Consuelo

**Front-end**

Frontend-ul Consuelo va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul Consuelo va fi operativ la [http://localhost:3000](http://localhost:3000)
