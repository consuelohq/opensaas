---
title: Aplicații Consuelo
description: Construiți și gestionați personalizările Consuelo sub formă de cod.
---
...
Aplicațiile vă permit să construiți și să gestionați personalizările Consuelo **sub formă de cod**. În loc să configurați totul prin interfața de utilizator (UI), vă definiți modelul de date și funcțiile de logică în cod — făcând mai rapidă construirea, mentenanța și implementarea în mai multe spații de lucru.
...
* Un spațiu de lucru Consuelo și o cheie API (creați una la https://app.twenty.com/settings/api-webhooks)
...
yarn consuelo entity:add

# Generate a typed Consuelo client and workspace entity types
yarn consuelo app:generate

# Watch your application's function logs
yarn consuelo function:logs

# Execute a function by name
yarn consuelo function:execute -n my-function -p '{"name": "test"}'

# Uninstall the application from the current workspace
yarn consuelo app:uninstall
...
* Creează fișiere de configurare și scripturi conectate la CLI-ul `consuelo`
...
* **package.json**: Declară numele aplicației, versiunea, motoarele (Node 24+, Yarn 4) și adaugă `twenty-sdk` plus un script `consuelo` care deleagă către CLI-ul local `consuelo`. Rulează `yarn consuelo help` pentru a lista toate comenzile disponibile.
...
* `yarn consuelo app:generate` va crea un folder `generated/` (client Consuelo tipizat + tipuri pentru spațiul de lucru).
* `yarn consuelo entity:add` va adăuga fișiere de definire a entităților în `src/` pentru obiectele, funcțiile, componentele front-end sau rolurile personalizate.

## Autentificare

The first time you run `yarn consuelo auth:login`, you'll be prompted for:
...
yarn consuelo auth:login --workspace my-custom-workspace

# List all configured workspaces
yarn consuelo auth:list

# Switch the default workspace (interactive)
yarn consuelo auth:switch

# Switch to a specific workspace
yarn consuelo auth:switch production

# Check current authentication status
yarn consuelo auth:status
```

Once you've switched workspaces with `yarn consuelo auth:switch`, all subsequent commands will use that workspace by default. Îl puteți totuși suprascrie temporar cu `--workspace <name>`.
...
* You can scaffold new objects using `yarn consuelo entity:add`, which guides you through naming, fields, and relationships.

<Note>
  **Câmpurile de bază sunt create automat.** Când definiți un obiect personalizat, Consuelo adaugă automat câmpuri standard precum `name`, `createdAt`, `updatedAt`, `createdBy`, `position` și `deletedAt`. Nu trebuie să le definiți în tabloul `fields` — adăugați doar câmpurile personalizate proprii.
</Note>
...
export default defineApplication({
  ...
  displayName: 'My Consuelo App',
  description: 'My first Consuelo app',
  icon: 'IconWorld',
...
export default defineRole({
  ...
  description: 'Default role for function Consuelo client',
...
* Vedeți un exemplu funcțional în aplicația Hello World: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
import Consuelo, { type Person } from '~/generated';

const handler = async (params: RoutePayload) => {
  const client = new Consuelo(); // generated typed client
...
* **Scaffolded**: Run `yarn consuelo entity:add` and choose the option to add a new logic function. Aceasta generează un fișier inițial cu un handler și o configurație.
* **Manual**: Creați un fișier nou `*.logic-function.ts` și folosiți `defineLogicFunction()`, urmând același model.

### Componente Front

Componentele Front vă permit să construiți componente React personalizate care sunt randate în interfața Consuelo. Utilizați `defineFrontComponent()` pentru a defini componente cu validare încorporată:
...
const MyWidget = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <p>This is a custom front component for Consuelo.</p>
    </div>
  );
};
...
* Componentele Front sunt componente React care sunt randate în contexte izolate în cadrul Consuelo.
...
* Components are built and synced automatically during `yarn consuelo app:dev`.

Puteți crea componente Front noi în două moduri:

* **Scaffolded**: Run `yarn consuelo entity:add` and choose the option to add a new front component.
* **Manual**: Creați un fișier nou `*.front-component.tsx` și folosiți `defineFrontComponent()`.

### Client tipizat generat

Run `yarn consuelo app:generate` to create a local typed client in `generated/` based on your workspace schema. Folosiți-l în funcțiile dvs.:

```typescript
import Consuelo from '~/generated';

const client = new Consuelo();
const { me } = await client.query({ me: { id: true, displayName: true } });
```

The client is re-generated by `yarn consuelo app:generate`. Rulați din nou după ce vă modificați obiectele sau când vă integrați într-un spațiu de lucru nou.

#### Acreditări la runtime în funcțiile de logică

Când funcția rulează pe Consuelo, platforma injectează acreditări ca variabile de mediu înainte de execuția codului:

* `TWENTY_API_URL`: URL-ul de bază al API-ului Consuelo către care țintește aplicația.
...
Explorați un exemplu minim, cap la cap, care demonstrează obiecte, funcții de logică, componente Front și declanșatoare multiple [aici](https://github.com/twentyhq/consuelo/tree/main/packages/twenty-apps/hello-world):
...
Now you can run all commands via `yarn consuelo <command>`, e.g. `yarn consuelo app:dev`, `yarn consuelo app:generate`, `yarn consuelo help`, etc.

## Depanare

* Authentication errors: run `yarn consuelo auth:login` and ensure your API key has the required permissions.
* Nu se poate conecta la server: verificați URL-ul API și că serverul Consuelo este accesibil.
* Types or client missing/outdated: run `yarn consuelo app:generate`.
* Dev mode not syncing: ensure `yarn consuelo app:dev` is running and that changes are not ignored by your environment.
