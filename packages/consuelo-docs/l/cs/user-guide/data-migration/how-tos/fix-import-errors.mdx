...
Po nahrání souboru a namapování sloupců Consuelo vaše data ověří:

1. **Proběhne validace** — Consuelo zkontroluje každý řádek na chyby
...
| --------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Duplicitní hodnoty zvýrazněné žlutě                                   | Hodnota již existuje v Consuelo nebo se ve vašem souboru vyskytuje dvakrát |
...
**Možnost 3: Nechte Consuelo aktualizovat existující záznam**
...
3. Consuelo aktualizuje existující záznam místo vytvoření duplicity
...
<Note>
  ...
  * Pokud ponecháte `email`, ale změníte `id` → ID bude aktualizováno
  ...
  Pokud se shoduje alespoň jeden jedinečný identifikátor, Consuelo záznam aktualizuje.
</Note>
...
3. Zkontrolujte, zda záznamy v Consuelo již existují
...
To znamená, že v Consuelo neexistuje společnost s tímto konkrétním identifikátorem.
...
Pokoušíte se propojit se záznamem, který v Consuelo neexistuje.
...
Sloupec ve vašem souboru nelze namapovat, protože pole v Consuelo neexistuje.
...
1. **Uživatel nepřijal pozvánku** — uživatel zatím v Consuelo neexistuje
2. **Používáte uživatelské ID ze starého systému** — Consuelo nedokáže spárovat ID z jiného systému
3. **Nesprávný formát e-mailu** — e-mail neodpovídá účtu uživatele v Consuelo

### Jak to opravit

1. Ujistěte se, že všichni uživatelé **přijali svou pozvánku** do vašeho pracovního prostoru Consuelo
2. Použijte **e-mailovou adresu** uživatele (ne jeho jméno nebo ID ze starého systému)
3. Použijte stejný e-mail, jaký použili k připojení do Consuelo
...
<Check>Formát domény přesně odpovídá formátu Consuelo</Check>
...
1. **Stáhněte si ukázkový soubor** — uvidíte přesný formát, který Consuelo očekává
