...
Pokud máte existující instanci Consuelo s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci Consuelo můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace Consuelo musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
**Pokud to nefunguje:**
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V Consuelo jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.
...
* Otevřete svou Consuelo aplikaci, přejděte na `/settings` a na levé dolní části obrazovky aktivujte přepínač **Pokročilé**.
* Vytvořte nový webhook.
* Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve Consuelo. Nastavte **Filtry** na `Companies` a `Created`.
...
Pokud provozujete Consuelo za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   * Potvrďte, že vaše reverzní proxy správně směruje požadavky na server Consuelo.
   * Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery Consuelo.
...
* Podpora komunity:
  ...
  Obraťte se na [komunitu Consuelo](https://github.com/consuelohq/opensaas/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.
