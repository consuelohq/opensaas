...
Tato příručka poskytuje podrobný návod k instalaci a konfiguraci aplikace Consuelo pomocí Docker Compose. Cílem je zjednodušit proces a zabránit běžným úskalím, která by mohla narušit vaše nastavení.
...
Nainstalujte nejnovější stabilní verzi Consuelo jedním příkazem:

```bash
bash <(curl -sL https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/scripts/install.sh)
```

Pro instalaci konkrétní verze nebo větve vypište:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/scripts/install.sh)
```
...
1. **Vytvořte soubor .env**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/consuelohq/opensaas/refs/heads/main/packages/twenty-docker/.env.example
   ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/consuelohq/opensaas/refs/heads/main/packages/twenty-docker/docker-compose.yml
```
...
Pokud hostujete Consuelo na svém vlastním počítači, otevřete svůj prohlížeč a přejděte na stránku [http://localhost:3000](http://localhost:3000).
...
### Otevření Consuelo pro Externí Přístup

Ve výchozím nastavení běží Consuelo na `localhost` na portu `3000`. Pro přístup přes externí doménu nebo IP adresu musíte nakonfigurovat `SERVER_URL` ve vašem `.env` souboru.
...
SSL (HTTPS) je potřebné pro správné fungování některých funkcí prohlížeče. I když tyto funkce mohou fungovat během místního vývoje (protože prohlížeče zacházejí s localhost jinak), správná konfigurace SSL je nutná při hostování Consuelo na běžné doméně.
...
Důrazně doporučujeme nastavit Consuelo za reverzní proxy se SSL ukončením pro optimální bezpečnost a funkčnost.
