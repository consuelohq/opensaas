...
1. V terminálu, na hostiteli, kde Consuelo běží, vypněte Consuelo: `docker compose down`
...
3. Opětovně zapněte Consuelo pomocí `docker compose up -d`
...
Upgradujte svou instanci Consuelo pro použití v0.55 image
...
Tento problém je specifický pro tuto verzi Consuelo a neměl by být vyžadován pro budoucí upgrady.
...
Měla by se vždy nacházet v rozmezí vaší aktuální instance Consuelo `major.minor` verze. Můžete zkontrolovat verzi instance na ovládacím panelu administrátora (na `/settings/admin-panel`, přístupné, pokud máte v databázi nastaveno uživatelské vlastnosti `canAccessFullAdminPanel` na true) nebo spuštěním `echo $APP_VERSION` ve vašem kontejneru `twenty-server`.

Chcete-li opravit asynchronizaci verzí pracovního prostoru, budete muset upgradovat z odpovídající verze consuelo podle souvisejícího průvodce upgradem po jednotlivých krocích, až do dosažení požadované verze.
...
Upgradujte svou instanci Consuelo pro použití v0.52 image
...
Budete muset ručně změnit verzi pracovního prostoru na `0.51.0` v databázi a upgradovat pomocí consuelo verze `0.52.11` podle jejího výše uvedeného průvodce upgradem.

### v0.50 až v0.51

Upgradujte svou instanci Consuelo pro použití v0.51 image
...
Upgradujte svou instanci Consuelo pro použití v0.50.0 image
...
Aktualizujte svůj místní `docker-compose.yml` pomocí [docker-compose.yml v0.50.0](https://github.com/consuelohq/opensaas/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)

### v0.43.0 až v0.44.0

Upgradujte svou instanci Consuelo pro použití v0.44.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.43.0 image
...
2. Upgradujte svůj `docker-compose.yml` na použití obrazu postgres:16 podle [docker-compose.yml](https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/docker-compose.yml) soubor.
...
Upgradujte svou instanci Consuelo pro použití v0.42.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.41.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.40.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.35.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.34.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.33.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.32.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.31.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.30.0 image

**Změna, která způsobí nekompatibilitu**:
Pro zvýšení výkonu nyní vyžaduje Consuelo konfiguraci cache Redis. Aktualizovali jsme náš [docker-compose.yml](https://raw.githubusercontent.com/consuelohq/opensaas/main/packages/twenty-docker/docker-compose.yml), aby to odrážel.
...
Upgradujte svou instanci Consuelo pro použití v0.24.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.23.0 image
...
Upgradujte svou instanci Consuelo pro použití v0.22.0 image
