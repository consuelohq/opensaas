---
title: Aplikace Consuelo
description: Vytvářejte a spravujte přizpůsobení Consuelo jako kód.
---
...
Aplikace vám umožňují vytvářet a spravovat přizpůsobení Consuelo **jako kód**. Místo konfigurace všeho přes uživatelské rozhraní definujete v kódu svůj datový model a logické funkce — což zrychluje vývoj, údržbu i nasazování do více pracovních prostorů.
...
* Node.js 24+ a Yarn 4
* Pracovní prostor Consuelo a klíč API (vytvořte si jej na https://app.consuelo.com/settings/api-webhooks)
...
<Note>
  **Základní pole jsou vytvořena automaticky.** Když definujete vlastní objekt, Consuelo automaticky přidá standardní pole jako `name`, `createdAt`, `updatedAt`, `createdBy`, `position` a `deletedAt`. Nemusíte je definovat v poli `fields` — přidejte pouze svá vlastní pole.
</Note>
...
* Podívejte se na funkční příklad v aplikaci Hello World: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
Frontendové komponenty vám umožňují vytvářet vlastní React komponenty, které se vykreslují v rozhraní Consuelo. K definování komponent s vestavěnou validací použijte `defineFrontComponent()`:
...
* Frontendové komponenty jsou React komponenty, které se vykreslují v izolovaných kontextech v rámci Consuelo.
...
Když vaše funkce běží na Consuelo, platforma před spuštěním kódu vloží přihlašovací údaje jako proměnné prostředí:

* `TWENTY_API_URL`: Základní URL Consuelo API, na které vaše aplikace cílí.
...
Prozkoumejte minimalistický end-to-end příklad, který demonstruje objekty, logické funkce, frontendové komponenty a více spouštěčů [zde](https://github.com/consuelohq/opensaas/tree/main/packages/twenty-apps/hello-world):
...
* Nelze se připojit k serveru: ověřte URL API a že je server Consuelo dosažitelný.
