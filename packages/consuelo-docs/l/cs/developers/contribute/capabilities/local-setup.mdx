...
description: Průvodce pro přispěvatele (nebo zvídavé vývojáře), kteří chtějí spustit Consuelo lokálně.
---
...
<Tabs>
  <Tab title="Linux a MacOS">
    Než nainstalujete a použijete Consuelo, ujistěte se, že máte na svém počítači nainstalovány následující balíčky:
...
Consuelo vyžaduje ke svému provozu Redis cache pro zajištění nejlepšího výkonu.
...
## Krok 8: Použijte Consuelo

**Frontend**

Frontend Consuelo bude spuštěný na [http://localhost:3001](http://localhost:3001).
Můžete se přihlásit pomocí výchozího demo účtu: `tim@apple.dev` (heslo: `tim@apple.dev`)

**Backend**

* Server Consuelo poběží na [http://localhost:3000](http://localhost:3000)
