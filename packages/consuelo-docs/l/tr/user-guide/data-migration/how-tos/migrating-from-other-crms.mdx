...
description: Verilerinizi herhangi bir CRM'den Consuelo'ye taşımak için adım adım kılavuz.
---

## Genel Bakış

Bu kılavuz, verilerinizi herhangi bir CRM'den Consuelo'ye taşıma sürecinde size yol gösterir. Süreç, verilerinizi denetlemeyi, Consuelo çalışma alanınızı hazırlamayı, mevcut sisteminizden dışa aktarmayı ve Consuelo'ye içe aktarmayı içerir.
...
Mevcut CRM'iniz ile Consuelo arasında bir eşleme belgesi oluşturun:
...
## Adım 3: Consuelo Çalışma Alanınızı Kurun

Verileri içe aktarmadan önce Consuelo çalışma alanınızı hazırlayın:
...
<Warning>
  **Kullanıcıları verileri içe aktarmadan ÖNCE davet edin.**
  ...
  Verileriniz kullanıcı referansları içeriyorsa (Hesap Sahibi, Atanan vb.), bu kullanıcıların içe aktarmadan önce Consuelo'de mevcut olması gerekir. Aksi halde bu ilişkiler eşlenemez.
</Warning>
...
Dışa aktarılan her CSV'yi bir hesap tablosu uygulamasında açın ve Consuelo için hazırlayın.
...
3. Consuelo'de halihazırda yinelenen olmadığını doğrulayın
...
<Warning>
  **Alan adı biçimi kritiktir.**
  ...
  `https://domain.com` kullanın (`domain.com` veya `www.domain.com` değil). Bu, Consuelo'nin biçimiyle eşleşir ve e-posta/takvim eşzamanlamasını bağladığınızda yinelenmeleri önler.
</Warning>
...
2. Kullanıcıların Consuelo çalışma alanınıza katılmak için kullandıklarıyla aynı e-posta adreslerini kullanın
...
## Adım 6: Consuelo'ye İçe Aktarın
...
* Consuelo, e-postaları kişi kayıtlarıyla eşzamanlamaya başlayacaktır
...
<AccordionGroup>
  <Accordion title="İş akışlarını otomatik olarak taşıyabilir miyim?">
    Şu anda mümkün değil. İş akışları Consuelo'de manuel olarak yeniden oluşturulmalıdır.
  </Accordion>
