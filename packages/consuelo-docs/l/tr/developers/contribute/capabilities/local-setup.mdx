---
title: Yerel Kurulum
description: Consuelo'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz.
---
...
<Tabs>
  <Tab title="Linux ve MacOS">
    Consuelo'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
Consuelo, en iyi performansı sağlamak için bir redis önbelleğe ihtiyaç duyar
...
## Adım 8: Consuelo'yi kullanın

**Ön Uç**

Consuelo'nin ön ucu [http://localhost:3001](http://localhost:3001) adresinde çalışıyor olacak.
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

* Consuelo'nin sunucusu [http://localhost:3000](http://localhost:3000) adresinde çalışır durumda olacak
