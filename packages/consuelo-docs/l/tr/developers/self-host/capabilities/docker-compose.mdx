...
Bu kılavuz, Docker Compose kullanarak Consuelo uygulamasını kurmak ve yapılandırmak için adım adım yönergeler sunar. Amaç, süreci sadeleştirmek ve kurulumunuzu bozabilecek yaygın sorunları önlemektir.
...
Tek bir komutla en son kararlı Consuelo sürümünü yükleyin:
...
```bash
VERSION=vx.y.z BRANCH=dal-adı bash <(curl -sL https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/scripts/install.sh)
```
...
Eğer Consuelo'i kendi bilgisayarınızda barındırıyorsanız, tarayıcınızı açın ve [http://localhost:3000](http://localhost:3000) adresine gidin.
...
### Consuelo'yi Dış Erişime Açma

Varsayılan olarak, Consuelo `localhost` üzerinde `3000` portunda çalışır. Harici bir alan adı veya IP adresi aracılığıyla erişim sağlamak için `.env` dosyanızda `SERVER_URL`'i yapılandırmanız gerekir.
...
Belirli tarayıcı özelliklerinin düzgün çalışabilmesi için SSL (HTTPS) gereklidir. Bu özellikler yerel geliştirme sırasında çalışabilirken (tarayıcılar localhost'u farklı şekilde ele alır), Consuelo'yi düzenli bir alanda barındırırken uygun bir SSL kurulumu gereklidir.
...
Optimum güvenlik ve işlevsellik için Consuelo'nin SSL sonlandırma ile bir ters proxy arkasında ayarlanmasını şiddetle öneririz.
