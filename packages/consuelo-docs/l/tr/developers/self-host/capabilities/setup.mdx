...
<Warning>
  **İlk kez mi kuruyorsunuz?** Consuelo'yi çalıştırmak için [Docker Compose kurulum kılavuzunu](/l/tr/developers/self-host/capabilities/docker-compose) izleyin, ardından yapılandırma için buraya dönün.
</Warning>

Consuelo, farklı dağıtım ihtiyaçlarına uygun **iki yapılandırma modu** sunar:
...
1. Consuelo örneğinize erişin (genellikle `http://localhost:3000`)
...
<Warning>
  ...
  [Tam teknik referans →](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-server/src/engine/core-modules/twenty-config/config-variables.ts)
</Warning>
...
Varsayılan olarak, Consuelo **tek çalışma alanı modunda** çalışır — kuruluşunuz için tek bir CRM örneğine ihtiyaç duyulan çoğu kendi barındırmalı dağıtım için idealdir.
...
* Her Consuelo örneği başına bir çalışma alanı
...
Aynı Consuelo örneği üzerinde birden fazla bağımsız ekibin kendi çalışma alanlarına ihtiyaç duyduğu SaaS benzeri dağıtımlar için çoklu çalışma alanı modunu etkinleştirin.
...
[İlgili kaynak kodunu görün](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Kullanıcıların Takvim ve Mesajlaşma API'sini kullanabilmesi için bir [Microsoft 365 Lisansı](https://admin.microsoft.com/Adminportal/Home) olması gerekir. Biri olmadan hesaplarını Consuelo'de eşitleyemeyecekler.
</Warning>
...
[İlgili kaynak kodunu görün](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
Consuelo, iş akışları ve özel mantık için mantıksal işlevleri destekler. Yürütme ortamı, `SERVERLESS_TYPE` ortam değişkeni aracılığıyla yapılandırılır.
...
<Note>
  `SERVERLESS_TYPE=DISABLED` kullanıldığında, bir mantıksal işlevi yürütmeye yönelik herhangi bir girişim bir hata döndürür. Bu, Consuelo'yi mantıksal işlev yetenekleri olmadan çalıştırmak istiyorsanız kullanışlıdır.
</Note>
