---
title: Приложения Consuelo
description: Создавайте и управляйте настройками Consuelo в виде кода.
---
...
Приложения позволяют создавать и управлять настройками Consuelo **в виде кода**. Вместо настройки всего через интерфейс вы определяете модель данных и логические функции в коде — так быстрее создавать, поддерживать и развёртывать в нескольких рабочих пространствах.
...
* Node.js 24+ и Yarn 4
* Рабочее пространство Consuelo и ключ API (создайте его на https://app.twenty.com/settings/api-webhooks)
...
* Создаёт файлы конфигурации и скрипты, подключённые к CLI `consuelo`
...
* **package.json**: Declares the app name, version, engines (Node 24+, Yarn 4), and adds `twenty-sdk` plus a `consuelo` script that delegates to the local `consuelo` CLI. Run `yarn consuelo help` to list all available commands.
...
* `yarn consuelo app:generate` will create a `generated/` folder (typed Consuelo client + workspace types).
* `yarn consuelo entity:add` will add entity definition files under `src/` for your custom objects, functions, front components, or roles.
...
Once you've switched workspaces with `yarn consuelo auth:switch`, all subsequent commands will use that workspace by default. Вы по-прежнему можете временно переопределить это с помощью `--workspace <name>`.
...
<Note>
  **Базовые поля создаются автоматически.** Когда вы определяете пользовательский объект, Consuelo автоматически добавляет стандартные поля, такие как `name`, `createdAt`, `updatedAt`, `createdBy`, `position` и `deletedAt`. Вам не нужно определять их в массиве `fields` — добавляйте только свои пользовательские поля.
</Note>
...
* См. рабочий пример в приложении Hello World: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/twentyhq/consuelo/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
* **Scaffolded**: Run `yarn consuelo entity:add` and choose the option to add a new logic function. Это создаёт стартовый файл с обработчиком и конфигурацией.
* **Вручную**: Создайте новый файл `*.logic-function.ts` и используйте `defineLogicFunction()`, следуя тому же шаблону.

### Фронт-компоненты

Фронт-компоненты позволяют создавать пользовательские компоненты React, которые рендерятся внутри интерфейса Consuelo. Используйте `defineFrontComponent()` для определения компонентов со встроенной валидацией:
...
* Фронт-компоненты — это компоненты React, которые рендерятся в изолированных контекстах внутри Consuelo.
...
Run `yarn consuelo app:generate` to create a local typed client in `generated/` based on your workspace schema. Используйте его в своих функциях:
...
The client is re-generated by `yarn consuelo app:generate`. Запускайте повторно после изменения ваших объектов или при подключении к новому рабочему пространству.

#### Учётные данные времени выполнения в логических функциях

Когда ваша функция запускается на Consuelo, платформа подставляет учётные данные как переменные окружения перед выполнением вашего кода:

* `TWENTY_API_URL`: Базовый URL API Consuelo, на который нацелено ваше приложение.
...
Ознакомьтесь с минимальным сквозным примером, демонстрирующим объекты, логические функции, фронт-компоненты и несколько триггеров, [здесь](https://github.com/twentyhq/consuelo/tree/main/packages/twenty-apps/hello-world):
...
* Не удаётся подключиться к серверу: проверьте URL API и доступность сервера Consuelo.
