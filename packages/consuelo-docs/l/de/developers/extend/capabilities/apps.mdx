...
* Ein Twenty-Workspace und ein API-Schlüssel (unter https://app.consuelo.com/settings/api-webhooks erstellen)
...
<Note>
  **Basisfelder werden automatisch erstellt.** Wenn Sie ein benutzerdefiniertes Objekt definieren, fügt Consuelo automatisch Standardfelder wie `name`, `createdAt`, `updatedAt`, `createdBy`, `position` und `deletedAt` hinzu. Sie müssen diese nicht in Ihrem `fields`-Array definieren — fügen Sie nur Ihre benutzerdefinierten Felder hinzu.
</Note>
...
* Ein funktionierendes Beispiel finden Sie in der Hello-World-App: [`packages/twenty-apps/hello-world/src/roles/function-role.ts`](https://github.com/consuelohq/opensaas/blob/main/packages/twenty-apps/hello-world/src/roles/function-role.ts).
...
* Frontend-Komponenten sind React-Komponenten, die in isolierten Kontexten innerhalb von Consuelo gerendert werden.
...
Wenn Ihre Funktion auf Consuelo läuft, injiziert die Plattform vor der Ausführung Ihres Codes Anmeldedaten als Umgebungsvariablen:
...
Ein minimales End-to-End-Beispiel, das Objekte, Logikfunktionen, Frontend-Komponenten und mehrere Trigger demonstriert, finden Sie [hier](https://github.com/consuelohq/opensaas/tree/main/packages/twenty-apps/hello-world):
