...
Nachdem Sie Ihre Datei hochgeladen und die Spalten zugeordnet haben, validiert Consuelo Ihre Daten:

1. **Validierung läuft** — Consuelo prüft jede Zeile auf Fehler
...
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| Doppelte Werte, gelb hervorgehoben                                             | Wert existiert bereits in Consuelo oder kommt in Ihrer Datei doppelt vor |
...
**Option 3: Consuelo den vorhandenen Datensatz aktualisieren lassen**
...
3. Consuelo aktualisiert den vorhandenen Datensatz, statt ein Duplikat zu erstellen
...
<Note>
  ...
  * Wenn Sie die `email` beibehalten, aber die `id` ändern → die ID wird aktualisiert
  ...
  Solange ein eindeutiger Bezeichner übereinstimmt, aktualisiert Consuelo den Datensatz.
</Note>
...
3. Prüfen Sie, ob Datensätze bereits in Consuelo existieren
...
Das bedeutet, dass es in Consuelo kein Unternehmen mit diesem spezifischen Bezeichner gibt.
...
Sie versuchen, auf einen Datensatz zu verweisen, der in Consuelo nicht existiert.
...
Eine Spalte in Ihrer Datei kann nicht zugeordnet werden, weil das Feld in Consuelo nicht existiert.
...
1. **Benutzer hat seine Einladung nicht angenommen** — der Benutzer existiert noch nicht in Consuelo
2. **Benutzer-ID aus altem System verwendet** — Consuelo kann IDs aus einem anderen System nicht zuordnen
...
3. Verwenden Sie dieselbe E-Mail, mit der sie Consuelo beigetreten sind
...
<Check>Domain-Format entspricht exakt dem Format von Consuelo</Check>
...
1. **Beispieldatei herunterladen** — sehen Sie das genaue Format, das Consuelo erwartet
