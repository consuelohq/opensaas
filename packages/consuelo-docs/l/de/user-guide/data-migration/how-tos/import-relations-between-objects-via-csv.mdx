...
### Häufige Beziehungen in Consuelo
...
Das übergeordnete Objekt ("Eins"-Seite) muss in Consuelo existieren, bevor Sie darauf verweisen können.
...
1. Exportieren Sie die übergeordneten Datensätze aus Consuelo
...
<Warning>
  **Das Domain-Format ist wichtig!**
  ...
  Verwenden Sie `https://domain.com` (nicht nur `domain.com`). Dies entspricht der Art, wie Consuelo Unternehmensdomains speichert, und verhindert Zuordnungsfehler.
</Warning>
...
Consuelo verknüpft automatisch jeden untergeordneten Datensatz mit dem passenden übergeordneten Datensatz.
...
| **Nicht vorhandene Datensätze**            | Der übergeordnete Datensatz existiert in Consuelo nicht     | Importieren Sie zunächst übergeordnete Datensätze oder prüfen Sie auf Tippfehler |
| **Unterschiedliche Groß-/Kleinschreibung** | `Acme.com` in der Datei, aber `acme.com` in Consuelo        | Achten Sie auf exakt übereinstimmende Groß-/Kleinschreibung                      |
