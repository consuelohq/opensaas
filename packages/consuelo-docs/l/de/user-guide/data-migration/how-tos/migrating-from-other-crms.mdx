...
description: Schritt-für-Schritt-Anleitung zur Migration Ihrer Daten von jedem CRM zu Consuelo.
---

## Übersicht

Diese Anleitung führt Sie durch die Migration Ihrer Daten von jedem CRM zu Consuelo. Der Prozess umfasst die Überprüfung Ihrer Daten, die Vorbereitung Ihres Twenty-Arbeitsbereichs, den Export aus Ihrem aktuellen System und den Import in Consuelo.
...
Erstellen Sie ein Zuordnungsdokument zwischen Ihrem aktuellen CRM und Consuelo:
...
<Warning>
  **Laden Sie Benutzer ein, BEVOR Sie Daten importieren.**
  ...
  Wenn Ihre Daten Benutzerreferenzen enthalten (Account Owner, Assignee usw.), müssen diese Benutzer vor dem Import in Consuelo existieren. Andernfalls können diese Beziehungen nicht zugeordnet werden.
</Warning>
...
Öffnen Sie jede exportierte CSV in einer Tabellenkalkulationsanwendung und bereiten Sie sie für Consuelo vor.
...
3. Überprüfen Sie, dass in Consuelo keine Duplikate vorhanden sind
...
<Warning>
  **Das Domain-Format ist entscheidend.**
  ...
  Verwenden Sie `https://domain.com` (nicht `domain.com` oder `www.domain.com`). Dies entspricht dem Format von Consuelo und verhindert Duplikate, wenn Sie die E-Mail-/Kalendersynchronisierung verbinden.
</Warning>
...
## Schritt 6: In Consuelo importieren
...
1. Navigieren Sie zum Objekt in Consuelo
...
* Consuelo beginnt, E-Mails mit Kontaktdatensätzen zu synchronisieren
...
<AccordionGroup>
  <Accordion title="Kann ich Workflows automatisch migrieren?">
    Derzeit nicht. Workflows müssen in Consuelo manuell neu erstellt werden.
  </Accordion>
...
Kontaktieren Sie [contact@consuelo.com](mailto:contact@consuelo.com) oder informieren Sie sich über unsere [Implementierungsservices](/l/de/user-guide/getting-started/capabilities/implementation-services).
