...
Wenn Sie eine Datei hochladen, analysiert Consuelo Ihre Spalten und versucht, sie bestehenden Feldern zuzuordnen.

### Automatische Zuordnung

Consuelo versucht, Spalten anhand der folgenden Kriterien abzugleichen:

* Spaltenüberschriften (genaue oder ähnliche Übereinstimmungen)
* Erkennung des Datentyps (Datum, Zahlen, E-Mails)
* Häufige Feldmuster

<Note>**Schneller Tipp:** Exportieren Sie ein paar Zeilen aus dem Objekt, das Sie importieren möchten. Die exportierte Datei enthält genau die Spaltennamen, die Consuelo erwartet, sodass die automatische Zuordnung beim Import reibungslos funktioniert.</Note>
...
Die Angabe einer `id` während des Imports ist optional. Consuelo erzeugt automatisch eine, wenn keine angegeben wird.
