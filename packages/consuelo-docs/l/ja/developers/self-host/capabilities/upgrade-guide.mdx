...
1. Consueloを実行しているホスト上のターミナルで、Consueloを停止します：`docker compose down`
...
3. `docker compose up -d` でConsueloを再起動します。
...
Consueloインスタンスをv0.55イメージにアップグレードします。
...
この問題は特定のConsueloバージョンに固有であり、今後のアップグレードでは不要なはずです。
...
それは常にあなたの現在のConsueloのインスタンスの `major.minor` バージョンの範囲内である必要があります。あなたのインスタンスバージョンは、管理パネル（ `/settings/admin-panel` にあり、データベース内でユーザーが `canAccessFullAdminPanel` プロパティを持っている場合にアクセス可能）にまたは `twenty-server` コンテナ内で `echo $APP_VERSION` を実行することで確認できます。

非同期化されたワークスペースバージョンを修正するには、対応するConsueloのバージョンから関連するアップグレードガイドに従って順次アップグレードし、それが目的のバージョンに到達するまで続ける必要があります。
...
Consueloインスタンスをv0.52イメージにアップグレードします。
...
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、consueloバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、consueloバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。

### v0.50からv0.51

Consueloインスタンスをv0.51イメージにアップグレードします。
...
Consueloインスタンスをv0.50.0イメージにアップグレードします。
...
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/twentyhq/consuelo/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/twentyhq/consuelo/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/twentyhq/consuelo/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)に更新してください。

### v0.43.0からv0.44.0

Consueloインスタンスをv0.44.0イメージにアップグレードします。
...
Consueloインスタンスをv0.43.0イメージにアップグレードします。
...
2. docker-compose.ymlを、新しいpostgres:16イメージを使用するようにアップグレードします。[docker-compose.yml](https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/docker-compose.yml)を参照してください。
...
Consueloインスタンスをv0.42.0イメージにアップグレードします。
...
Consueloインスタンスをv0.41.0イメージにアップグレードします。
...
Consueloインスタンスをv0.40.0イメージにアップグレードします。
...
Consueloインスタンスをv0.35.0イメージにアップグレードします。
...
Consueloインスタンスをv0.34.0イメージにアップグレードします。
...
Consueloインスタンスをv0.33.0イメージにアップグレードします。
...
このバージョンから、DB用のtwenty-postgresイメージは廃止され、twenty-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のtwenty-postgresイメージは廃止され、twenty-postgres-spiloが代わりに使用されるようになりました。
twenty-postgresイメージを使い続けたい場合は、docker-compose.yml内の`Consuelo/twenty-postgres:${TAG}`を`Consuelo/twenty-postgres`に置き換えるだけで済みます。

### v0.31.0からv0.32.0

Consueloインスタンスをv0.32.0イメージにアップグレードします。
...
Consueloインスタンスをv0.31.0イメージにアップグレードします。
...
Consueloインスタンスをv0.30.0イメージにアップグレードします。

**破壊的な変更**:
パフォーマンスを向上させるために、ConsueloはRedisキャッシュの設定を必要とするようになりました。 これを反映した[docker-compose.yml](https://raw.githubusercontent.com/twentyhq/consuelo/main/packages/twenty-docker/docker-compose.yml)を更新しました。
...
Consueloインスタンスをv0.24.0イメージにアップグレードします。
...
Consueloインスタンスをv0.23.0イメージにアップグレードします。
...
Consueloインスタンスをv0.22.0イメージにアップグレードします。
