...
すでに稼働中のConsueloインスタンスに本番データがある場合、これらの手順に従うとデータベースが完全に削除されますので、絶対に従わないでください。

Consueloを初めてインストールする際には、デフォルトのデータベースパスワードを変更する必要があるかもしれません。
...
Consueloのインストール中に、postgresデータベースを適切なスキーマ、拡張、およびユーザーで準備する必要があります。
...
あなたのマシンのRAMを最も使用しているプロセスを調査してください。 Consueloでは、一部のVScode拡張機能が多くのRAMを使用していることに気付き、一時的に無効化しました。
...
* Consueloアプリを開き、`/settings`に移動し、画面左下の**Advanced**トグルを有効にします。
* 新しいWebhookを作成します。
* **Your Unique Webhook URL**をConsueloの**Endpoint Url**フィールドに貼り付けます。 **Filters**を`Companies`と`Created`に設定します。
...
リバースプロキシの背後でConsueloを実行し、接続問題が発生している場合:
...
2. **リバースプロキシ設定の確認:**
   ...
   * リバースプロキシがConsueloサーバーへのリクエストを正しく転送していることを確認してください。
   * `X-Forwarded-For`や`X-Forwarded-Proto`といったヘッダーが正しく設定されていることを確認してください。

3. **サービスの再起動:**

   変更を行った後に、リバースプロキシとConsueloコンテナの両方を再起動します。
...
* コミュニティサポート:
  ...
  [Consueloコミュニティ](https://github.com/twentyhq/consuelo/issues)または[サポートチャンネル](https://discord.gg/cx5n4Jzs57)に連絡してサポートを受けてください。
