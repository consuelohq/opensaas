...
export const READER_SHELL_VERSION = '1.1.0';
...
function renderSection(section: ReaderSection): string {
  ...
  const flat = !body && !section.callout && !section.metrics?.length && !section.flow?.length && !section.table && !section.timeline?.length && !section.details?.length && !section.ranges?.length && !section.comparisons?.length && !!section.cards?.length;
