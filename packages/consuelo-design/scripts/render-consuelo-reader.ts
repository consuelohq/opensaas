...
function ledgerGroupMarkdown(group: ReaderLedgerGroup): string {
  ...
  for (const item of group.items) {
    const marker = item.status === 'done' ? 'x' : item.status === 'current' ? '>' : ' ';
    lines.push(`- [${marker}] ${item.text}`);
  }
  return lines.join('\n');
}

function renderLedgerCopyButton(group: ReaderLedgerGroup): string {
  return `<button class="task-copy-button" type="button" data-copy-markdown="${escapeHtml(ledgerGroupMarkdown(group))}" aria-label="Copy ${escapeHtml(group.title)} as Markdown" title="Copy task as Markdown"><span class="task-copy-label">Copy</span></button>`;
}

function renderLedgerGroups(groups: ReaderLedgerGroup[]): string {
  return groups.map((group) => `<article class="checklist-group"><div class="checklist-heading"><h3>${escapeHtml(group.title)}${group.tag ? ` <span>${escapeHtml(group.tag)}</span>` : ''}</h3>${renderLedgerCopyButton(group)}</div><ul class="checklist-items">${group.items.map((item) => `<li><i class="check-box ${escapeHtml(item.status)}">${item.status === 'done' ? '✓' : item.status === 'current' ? '•' : ''}</i><span>${escapeHtml(item.text)}</span></li>`).join('')}</ul></article>`).join('');
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } html { min-height:100%; scroll-behavior:smooth; background:var(--paper); } body { min-height:100dvh; margin:0; font-family:var(--sans); color:var(--ink); background:var(--paper); font-size:16px; line-height:1.5; } a { color:inherit; } p, li { color:color-mix(in srgb, var(--ink) 76%, var(--muted)); }
    ...
    .ship-checklist { display:grid; gap:14px; } .checklist-group h3 span { color:var(--muted); font:700 11px/1 var(--mono); text-transform:uppercase; letter-spacing:.08em; } .checklist-heading { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; margin-bottom:10px; } .task-copy-button { flex:0 0 auto; border:1px solid var(--line); border-radius:999px; background:var(--soft); color:var(--muted); cursor:pointer; padding:7px 11px; font:700 10px/1 var(--mono); letter-spacing:.08em; text-transform:uppercase; transition:transform .16s ease, border-color .16s ease, color .16s ease, background .16s ease; } .task-copy-button[data-copied="true"] { background:var(--forest); border-color:var(--forest); color:var(--paper); } .task-copy-button[data-copied="error"] { background:var(--terracotta); border-color:var(--terracotta); color:#211b17; } .checklist-items { list-style:none; margin:0; padding:0; display:grid; gap:10px; } .checklist-items li { display:grid; grid-template-columns:18px 1fr; gap:10px; align-items:start; color:var(--muted); } .check-box { width:15px; height:15px; border-radius:4px; border:1px solid rgba(47,91,79,.34); margin-top:4px; display:inline-grid; place-items:center; color:var(--paper); font:700 10px/1 var(--mono); font-style:normal; } .check-box.done { background:var(--forest); border-color:var(--forest); } .check-box.current { background:var(--terracotta); border-color:var(--terracotta); } .check-box.todo { background:transparent; } .footer-meta { padding:42px 0 70px; color:var(--muted); font:500 12px/1.6 var(--mono); border-top:1px solid var(--line); }
...
<body data-reader-shell-version="${escapeHtml(READER_SHELL_VERSION)}" data-reader-shell-template="${escapeHtml(content.template)}">
  ...
  <script>
    ...
    function copyTaskMarkdown(button){ const text = button.dataset.copyMarkdown || ''; if (!text) return; const label = button.textContent || 'Copy'; const ok = () => { button.dataset.copied = 'true'; button.textContent = 'Copied'; setTimeout(() => { button.dataset.copied = 'false'; button.textContent = label; }, 1200); }; const fail = () => { button.dataset.copied = 'error'; button.textContent = 'Copy failed'; setTimeout(() => { button.dataset.copied = 'false'; button.textContent = label; }, 1600); }; if (navigator.clipboard?.writeText) { navigator.clipboard.writeText(text).then(ok, fail); return; } const textarea = document.createElement('textarea'); textarea.value = text; textarea.setAttribute('readonly',''); textarea.style.position = 'fixed'; textarea.style.left = '-9999px'; document.body.appendChild(textarea); textarea.select(); document.execCommand('copy') ? ok() : fail(); textarea.remove(); }
    document.querySelectorAll('.task-copy-button').forEach((button) => button.addEventListener('click', (event) => { event.preventDefault(); event.stopPropagation(); copyTaskMarkdown(button); }));
