...
function sectionNavigationItems(content: ConsueloReaderContent): ReaderLink[] {
  const sectionLinks = content.sections.map((section) => ({ label: section.eyebrow ?? section.title, href: `#${section.id}` }));
...
function renderLedgerGroups(groups: ReaderLedgerGroup[]): string {
  return groups.map((group) => {
    ...
    return `<article class="checklist-group"><div class="checklist-heading"><h3>${escapeHtml(group.title)}${group.tag ? ` <span>${escapeHtml(group.tag)}</span>` : ''}</h3><button class="task-copy-button" type="button" data-copy-markdown="${escapeAttribute(markdown)}" aria-label="Copy task as Markdown" title="Copy task as Markdown"><span class="task-copy-icon" aria-hidden="true"></span></button></div><ul class="checklist-items">${group.items.map((item) => `<li><i class="check-box ${escapeHtml(item.status)}">${item.status === 'done' ? '✓' : item.status === 'current' ? '•' : ''}</i><span>${escapeHtml(item.text)}</span></li>`).join('')}</ul></article>`;
  }).join('');
}
...
function sectionRailLinks(items: ReaderLink[]): string {
  return items.map((item, index) => `<button type="button" class="reader-section-line" data-target="${escapeHtml(item.href)}" data-reader-section-target="${escapeHtml(item.href)}" data-reader-section-title="${escapeHtml(item.label)}" aria-label="Open section drawer at ${escapeHtml(item.label)}"><i class="reader-section-line-mark" aria-hidden="true"></i><span class="reader-section-line-label">${escapeHtml(item.label)}</span><b class="reader-section-line-index">${String(index + 1).padStart(2, '0')}</b></button>`).join('');
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } html { min-height:100%; scroll-behavior:smooth; background:var(--paper); } body { min-height:100dvh; margin:0; font-family:var(--sans); color:var(--ink); background:var(--paper); font-size:16px; line-height:1.5; } a { color:inherit; } p, li { color:color-mix(in srgb, var(--ink) 76%, var(--muted)); }
    ...
    .reader-section-rail { position:fixed; z-index:45; right:18px; top:50%; transform:translateY(-50%); display:grid; gap:4px; width:36px; max-height:min(72vh,680px); overflow-y:auto; overflow-x:visible; padding:4px 4px; border:1px solid transparent; border-radius:18px; background:transparent; scrollbar-width:none; transition:width .18s ease, background .18s ease, border-color .18s ease, box-shadow .18s ease; } .reader-section-rail::-webkit-scrollbar { width:0; height:0; } .reader-section-line { position:relative; width:28px; min-height:8px; display:grid; grid-template-columns:28px 0 0; gap:0; align-items:center; justify-self:end; padding:2px 0; border:0; border-radius:999px; background:transparent; color:var(--ink); cursor:pointer; text-align:left; overflow:hidden; -webkit-tap-highlight-color:transparent; transition:width .18s ease, padding .18s ease, background .18s ease; } .reader-section-line-mark { display:block; justify-self:end; width:24px; height:2px; border-radius:999px; background:var(--line-strong); transition:width .18s ease, background .18s ease; } .reader-section-line-label { min-width:0; opacity:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; transform:translateX(6px); color:var(--ink); font:700 13px/1.05 var(--sans); letter-spacing:-.02em; transition:opacity .16s ease, transform .16s ease; } .reader-section-line-index { opacity:0; color:var(--muted); font:700 10px/1 var(--mono); text-align:right; transition:opacity .16s ease; } .reader-section-rail:hover,.reader-section-rail:focus-within { width:min(300px, calc(100vw - 58px)); padding:10px; border-color:var(--line); background:color-mix(in srgb, var(--surface) 92%, transparent); box-shadow:var(--shadow); backdrop-filter:blur(16px); } .reader-section-rail:hover .reader-section-line,.reader-section-rail:focus-within .reader-section-line { width:100%; min-height:34px; grid-template-columns:34px minmax(0,1fr) 28px; gap:10px; padding:8px 10px; background:var(--soft); } .reader-section-rail:hover .reader-section-line-mark,.reader-section-rail:focus-within .reader-section-line-mark { width:22px; } .reader-section-rail:hover .reader-section-line-label,.reader-section-rail:focus-within .reader-section-line-label,.reader-section-rail:hover .reader-section-line-index,.reader-section-rail:focus-within .reader-section-line-index { opacity:1; transform:none; } .reader-section-line.active .reader-section-line-mark { width:28px; background:var(--terracotta); } .reader-section-rail:hover .reader-section-line.active,.reader-section-rail:focus-within .reader-section-line.active { background:color-mix(in srgb, var(--terracotta) 14%, var(--soft)); }
    .reader-section-drawer-toggle { display:none; } .reader-section-drawer[hidden] { display:none !important; } .reader-section-drawer { position:fixed; inset:0; z-index:70; display:grid; place-items:end stretch; background:rgba(0,0,0,.52); backdrop-filter:blur(10px); } .reader-section-drawer-panel { align-self:end; width:100%; max-height:min(84dvh, 760px); overflow-y:auto; padding:0 24px calc(92px + env(safe-area-inset-bottom,0px)); border:1px solid var(--line); border-bottom:0; border-radius:32px 32px 0 0; background:var(--surface); box-shadow:0 -20px 80px rgba(0,0,0,.34); } .reader-section-drawer-head { position:sticky; top:0; z-index:1; display:flex; align-items:center; justify-content:space-between; gap:16px; margin:0 -24px 12px; padding:24px; border-bottom:1px solid var(--line); background:color-mix(in srgb, var(--surface) 94%, transparent); backdrop-filter:blur(14px); } .reader-section-drawer-head p { margin:0; color:var(--ink); font:800 26px/1 var(--sans); letter-spacing:-.04em; text-transform:none; } .reader-section-drawer-head button { width:52px; height:52px; border:1px solid var(--line); border-radius:999px; background:var(--paper); color:var(--ink); cursor:pointer; font:400 30px/1 var(--sans); box-shadow:var(--shadow); } .reader-section-drawer-list { display:grid; gap:0; padding:6px 0 18px; } .reader-section-drawer-row { display:grid; grid-template-columns:44px 1fr; gap:14px; align-items:center; min-height:56px; padding:14px 0; border:0; border-bottom:1px solid var(--line); background:transparent; color:var(--ink); text-align:left; cursor:pointer; } .reader-section-drawer-row i { color:var(--muted); font:700 11px/1 var(--mono); font-style:normal; } .reader-section-drawer-row span { font:700 24px/1.1 var(--sans); letter-spacing:-.04em; }
    ...
    .ship-checklist { display:grid; gap:14px; } .checklist-heading { position:relative; display:grid; grid-template-columns:minmax(0,1fr) auto; gap:14px; align-items:start; padding-right:2px; } .checklist-group h3 span { color:var(--muted); font:700 11px/1 var(--mono); text-transform:uppercase; letter-spacing:.08em; } .task-copy-button { width:32px; height:32px; display:grid; place-items:center; border:1px solid var(--line); border-radius:9px; background:var(--soft); color:var(--ink); cursor:pointer; transition:transform .16s ease, border-color .16s ease, background .16s ease; } .task-copy-button:hover,.task-copy-button:focus-visible { transform:translateY(-1px); border-color:var(--line-strong); background:color-mix(in srgb, var(--terracotta) 12%, var(--soft)); outline:none; } .task-copy-button[data-copied="true"] { border-color:var(--terracotta); background:color-mix(in srgb, var(--terracotta) 18%, var(--soft)); } .task-copy-icon { position:relative; width:16px; height:16px; display:block; } .task-copy-icon::before,.task-copy-icon::after { content:""; position:absolute; width:10px; height:10px; border:2px solid currentColor; border-radius:3px; background:transparent; } .task-copy-icon::before { left:1px; top:5px; opacity:.72; } .task-copy-icon::after { left:5px; top:1px; background:var(--surface); } .reader-copy-toast[hidden] { display:none !important; } .reader-copy-toast { position:fixed; z-index:90; left:50%; bottom:calc(96px + env(safe-area-inset-bottom,0px)); transform:translateX(-50%) translateY(8px); padding:10px 14px; border:1px solid var(--line); border-radius:999px; background:rgba(32,32,32,.88); color:#fff; box-shadow:0 14px 48px rgba(0,0,0,.24); opacity:0; pointer-events:none; backdrop-filter:blur(16px); font:700 13px/1 var(--sans); transition:opacity .16s ease, transform .16s ease; } .reader-copy-toast[data-visible="true"] { opacity:1; transform:translateX(-50%) translateY(0); } .checklist-items { list-style:none; margin:0; padding:0; display:grid; gap:10px; } .checklist-items li { display:grid; grid-template-columns:18px 1fr; gap:10px; align-items:start; color:var(--muted); } .check-box { width:15px; height:15px; border-radius:4px; border:1px solid rgba(47,91,79,.34); margin-top:4px; display:inline-grid; place-items:center; color:var(--paper); font:700 10px/1 var(--mono); font-style:normal; } .check-box.done { background:var(--forest); border-color:var(--forest); } .check-box.current { background:var(--terracotta); border-color:var(--terracotta); } .check-box.todo { background:transparent; } .footer-meta { padding:42px 0 70px; color:var(--muted); font:500 12px/1.6 var(--mono); border-top:1px solid var(--line); }
    ...
    @media (max-width: 980px) { .reader-section-rail { right:12px; } .hero-grid,.grid-2,.grid-3,.metric-grid,.range-grid,.flow-row,.decision-grid { grid-template-columns:1fr; } .spec-map { grid-template-columns:repeat(2, minmax(0,1fr)); } }
    @media (max-width: 680px) { .reader-section-rail { right:7px; width:30px; max-height:min(58vh,390px); gap:4px; padding:4px 2px; } .reader-section-rail:hover,.reader-section-rail:focus-within { width:30px; padding:4px 2px; border-color:transparent; background:transparent; box-shadow:none; backdrop-filter:none; } .reader-section-line,.reader-section-rail:hover .reader-section-line,.reader-section-rail:focus-within .reader-section-line { width:26px; min-height:7px; grid-template-columns:26px 0 0; gap:0; padding:1px 0; background:transparent; } .reader-section-line-mark,.reader-section-rail:hover .reader-section-line-mark,.reader-section-rail:focus-within .reader-section-line-mark { width:22px; height:2px; } .reader-section-line.active .reader-section-line-mark { width:26px; transform:none; background:var(--terracotta); } .reader-section-line-label,.reader-section-line-index,.reader-section-rail:hover .reader-section-line-label,.reader-section-rail:focus-within .reader-section-line-label,.reader-section-rail:hover .reader-section-line-index,.reader-section-rail:focus-within .reader-section-line-index { opacity:0; width:0; overflow:hidden; } .reader-section-drawer-toggle { display:none; } .reader-section-drawer-head button { position:fixed; left:24px; bottom:calc(82px + env(safe-area-inset-bottom,0px)); z-index:2; } .container { width:min(100vw - 26px, 1120px); } .reader-nav { top:14px; } .reader-nav-shell { width:calc(100vw - 24px); grid-template-columns:minmax(0,1fr) auto; gap:10px; padding-left:16px; } .reader-brand { position:relative; z-index:2; display:block; min-width:0; } .reader-links { display:none; } .reader-nav-task { justify-self:end; } .reader-progress { width:52px; } .hero { min-height:78vh; padding-top:106px; } h1 { font-size:clamp(48px, 12vw, 88px); letter-spacing:-.055em; } h2 { font-size:clamp(38px, 13vw, 64px); } section { padding:54px 0; } .spec-map { grid-template-columns:1fr; } .meta-grid { grid-template-columns:1fr; } .hero-card,.section-content,.callout,.matrix,.diagram { padding:20px; } .section-content.flat-content { padding:0; } table, thead, tbody, tr, td { display:block; width:100%; } thead { display:none; } tr { padding:14px 0; border-bottom:1px solid var(--line); } tr:last-child { border-bottom:0; } td { display:grid; grid-template-columns:118px minmax(0,1fr); gap:14px; padding:10px 0; border:0; overflow-wrap:anywhere; } td::before { content:attr(data-label); color:var(--muted); font:700 10px/1.2 var(--mono); letter-spacing:.1em; text-transform:uppercase; } .phase { grid-template-columns:1fr; } .phase-index { font-size:20px; } }
    @media (prefers-color-scheme: dark) { :root { color-scheme: dark; --paper:#202020; --ink:#f1f1f1; --surface:#272727; --muted:#b6b6b6; --line:rgba(255,255,255,.14); --line-strong:rgba(255,255,255,.28); --soft:rgba(255,255,255,.06); --terracotta:#ff8a63; --forest:#d5d5d5; --shadow:0 0 0 1px rgba(255,255,255,.04), 0 22px 70px rgba(0,0,0,.18); } html, body { background:var(--paper); color:var(--ink); } p, li, td { color:rgba(241,241,241,.78); } .hero-card,.section-content,.card,.checklist-group,.callout,.metric,.matrix,.diagram,.phase,.decision,.range-card,.comparison { background:rgba(39,39,39,.72); border-color:var(--line); box-shadow:var(--shadow); } .reader-nav-shell,.reader-back-to-top-progress,.reader-resume { background:rgba(32,32,32,.88); border-color:rgba(255,255,255,.16); box-shadow:0 12px 42px rgba(0,0,0,.22); } .reader-brand,.reader-links a,button { color:var(--ink); } .reader-links a,.reader-nav-task { color:rgba(241,241,241,.72); } .spec-map a,.flow-node,.meta-item { background:rgba(255,255,255,.045); border-color:var(--line); color:var(--ink); } .tag { border-color:var(--line); color:var(--muted); } .check-box { border-color:rgba(255,255,255,.52); background:rgba(255,255,255,.08); box-shadow:inset 0 0 0 1px rgba(255,255,255,.16), 0 0 0 1px rgba(0,0,0,.18); } .check-box.todo { border-color:rgba(255,255,255,.48); background:rgba(255,255,255,.075); } .check-box.current,.check-box.done { border-color:#ff8a63; background:#ff8a63; color:#202020; } }
  </style>
</head>
<body data-reader-shell-version="${escapeHtml(READER_SHELL_VERSION)}" data-reader-shell-template="${escapeHtml(content.template)}">
  ...
  <div class="reader-copy-toast" data-copy-toast hidden data-no-tap-scroll>Copied</div>
  ...
  <script>
    ...
    function showCopyToast(message = 'Copied'){ if (!copyToast) return; copyToast.textContent = message; copyToast.hidden = false; copyToast.dataset.visible = 'true'; clearTimeout(copyToastTimer); copyToastTimer = window.setTimeout(() => { delete copyToast.dataset.visible; copyToast.hidden = true; }, 1400); }
    ...
    function copySelectedText(){ if (Date.now() < suppressSelectionCopyUntil) return; const selection = window.getSelection(); const text = selection && !selection.isCollapsed ? selection.toString().trim() : ''; if (!text) return; window.__readerLastFindNeedle = text; writeClipboard(text).then((ok) => { if (!ok) return; selection.removeAllRanges(); showCopyToast('Copied'); }); }
