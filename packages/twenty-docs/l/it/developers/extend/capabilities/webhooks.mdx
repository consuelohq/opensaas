...
I webhook inviano dati ai tuoi sistemi in tempo reale quando si verificano eventi in Consuelo — senza necessità di polling. Usali per mantenere sincronizzati i sistemi esterni, attivare automazioni o inviare avvisi.
...
Consuelo invia webhook per questi tipi di eventi:
...
Consuelo firma ogni richiesta webhook per motivi di sicurezza. Convalida le firme per assicurarti che le richieste siano autentiche.
...
| **Trigger webhook del Workflow** | IN        | Ricevi dati in Consuelo da sistemi esterni                                 |
