...
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать Consuelo локально.
---
...
<Tabs>
  <Tab title="Linux и MacOS">
    Прежде чем установить и использовать Consuelo, убедитесь, что у вас установлено следующее:
...
Consuelo требует кэша Redis для обеспечения наилучшей производительности
...
## Шаг 8: Использовать Consuelo

**Frontend**

Фронтенд Consuelo будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер Consuelo будет работать на [http://localhost:3000](http://localhost:3000).
