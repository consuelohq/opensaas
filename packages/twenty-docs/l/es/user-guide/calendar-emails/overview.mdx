...
description: Conecta tus cuentas de correo electrónico y calendario a Consuelo.
...
4. Autorizar a Consuelo para acceder a tu Gmail y Calendario de Google
...
4. Autorizar a Consuelo para acceder a tu Outlook y Calendario de Microsoft
5. Configura los ajustes de sincronización del correo electrónico (visibilidad, creación automática) → haz clic en **Siguiente**
6. Configura los ajustes de sincronización del calendario (visibilidad, creación automática) → haz clic en **Añadir cuenta**
7. Tus correos y eventos del calendario comenzarán a sincronizarse automáticamente
...
<Warning>
  Solo se pueden conectar verdaderos buzones (por ejemplo, soporte@dominio.com con su propia bandeja de entrada). Los alias de correo electrónico que reenvían a otro buzón no se pueden conectar a Consuelo.
</Warning>
...
<Note>Cuando está habilitado, los contactos se vinculan automáticamente a sus registros de Empresa en función de su dominio de correo electrónico. Si la Empresa aún no existe, Consuelo la crea para ti.</Note>

### Controla qué correos se sincronizan con la selección de carpetas de mensajes

Controle qué carpetas de correo electrónico se sincronizan con Consuelo:
...
**Nota**: No proporcionamos una dirección de correo CC para la sincronización selectiva. En su lugar, usa la función de carpeta de mensajes para lograr el mismo nivel de control sobre qué correos sincronizar con Consuelo.
...
<Note>
  **Tiempo de la sincronización inicial**: La sincronización del calendario se completa rápidamente (normalmente en unos minutos), mientras que la sincronización del correo electrónico tarda más en el caso de buzones grandes. Consuelo importa correos electrónicos a aproximadamente **400 mensajes por minuto** (limitado por el límite de tasa de la API de Gmail), por lo que un buzón con 10 000 correos electrónicos tarda unos 25 minutos. No te preocupes si ves que los contactos de eventos del calendario aparecen antes que tus contactos de correo electrónico; es un comportamiento normal.
</Note>
