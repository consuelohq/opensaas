...
After uploading your file and mapping columns, Consuelo validates your data:

1. **Validation runs** — Consuelo checks each row for errors
...
| --------------------------------------------------------------------- | ------------------------------------------------------------ |
| Duplicate values highlighted in yellow                                | Value already exists in Consuelo or appears twice in your file |
...
**Option 3: Let Consuelo update the existing record**

1. Ensure your file includes a unique identifier (`email`, `domain`, or `id`)
2. Map the unique identifier field
3. Consuelo will update the existing record instead of creating a duplicate
...
<Note>
  ...
  * If you keep the `email` but change the `id` → the id will be updated
  ...
  As long as one unique identifier matches, Consuelo updates the record.
</Note>
...
3. Check if records already exist in Consuelo
...
This means there is no Company in Consuelo with that specific identifier.
...
You're trying to link to a record that doesn't exist in Consuelo.
...
A column in your file can't be mapped because the field doesn't exist in Consuelo.
...
1. **User hasn't accepted their invitation** — the user doesn't exist in Consuelo yet
2. **Using user ID from old system** — Consuelo can't match IDs from another system
3. **Wrong email format** — the email doesn't match the user's Consuelo account

### How to Fix

1. Ensure all users have **accepted their invitation** to your Consuelo workspace
2. Use the user's **email address** (not their name or old system ID)
3. Use the same email they used to join Consuelo
...
<Check>Domain format matches Consuelo's format exactly</Check>
...
1. **Download the sample file** — see the exact format Consuelo expects
