...
O pai (lado "um") deve existir no Consuelo antes de poder referenciá-lo.
...
1. Exporte os registos pai do Consuelo
...
<Warning>
  **O formato do domínio é importante!**
  ...
  Use `https://domain.com` (não apenas `domain.com`). Isto corresponde à forma como a Consuelo armazena domínios de Empresas e evita erros de correspondência.
</Warning>
...
A Consuelo ligará automaticamente cada registo filho ao pai correspondente.
...
| **Registos inexistentes**                      | O registo pai não existe no Consuelo               | Importe primeiro os registos pai ou verifique se há erros ortográficos |
| **Incompatibilidade de maiúsculas/minúsculas** | `Acme.com` no ficheiro mas `acme.com` no Consuelo  | Garanta correspondência exata de maiúsculas/minúsculas                 |
...
<AccordionGroup>
  <Accordion title="E se o meu registo pai não tiver um domínio ou email?">
    ...
    1. Use o `id` da Consuelo (exporte os registos pai para obter os respetivos IDs)
