...
description: Fix the most common embedded GoHighLevel dialer issues.
---

Use this guide when the embedded widget loads, signs in, or calls incorrectly inside GoHighLevel.

## Widget does not appear in the GHL sidebar

**Check these first:**

1. verify the Consuelo app is installed for the correct GHL location
2. refresh GoHighLevel with a hard reload
...
1. refresh the GoHighLevel page first
2. reopen the embedded dialer from the sidebar
3. reconnect the app if your team manages the connection from a GHL integration screen
4. clear GoHighLevel cookies and log in again if the session looks stale

## Embedded-only warning or redirect to the main app

This usually means the widget was opened directly instead of through GoHighLevel.

Fix:

- open the dialer from the GHL sidebar
...
## Calls do not appear in GoHighLevel

Call logging is most reliable when the call started from a GHL contact.

If logs are missing:

1. confirm the contact exists in the current GHL location
...
- the GHL location where it happened
