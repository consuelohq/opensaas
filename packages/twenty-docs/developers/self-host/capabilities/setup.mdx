...
<Warning>
**First time installing?** Follow the [Docker Compose installation guide](/developers/self-host/capabilities/docker-compose) to get Consuelo running, then return here for configuration.
</Warning>

Consuelo offers **two configuration modes** to suit different deployment needs:
...
1. Access your Consuelo instance (usually `http://localhost:3000`)
...
By default, Consuelo runs in **single-workspace mode** — ideal for most self-hosted deployments where you need one CRM instance for your organization.
...
- One workspace per Consuelo instance
...
Enable multi-workspace mode for SaaS-like deployments where multiple independent teams need their own workspaces on the same Consuelo instance.
...
<Warning>
Users must have a [Microsoft 365 Licence](https://admin.microsoft.com/Adminportal/Home) to be able to use the Calendar and Messaging API. They will not be able to sync their account on Consuelo without one.
</Warning>
...
Consuelo supports logic functions for workflows and custom logic. The execution environment is configured via the `SERVERLESS_TYPE` environment variable.
