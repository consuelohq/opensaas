...
Wait reason: GitHub checks for PR #1180 are in progress after retargeting to main.
Duration: poll every 15s for up to 8 attempts.
Resume action: check PR #1180 status rollup.
Expected signal: all required checks pass or only non-blocking skips remain.
Fallback: report failing/pending checks and do not merge.
